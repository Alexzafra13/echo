...
export async function createTestApp(): Promise<{
  ...
  const redis = moduleFixture.get<RedisService>(RedisService);

  return { app, module: moduleFixture, drizzle, redis };
}
...
/**
 * Limpia tablas del scanner y el lock distribuido de Redis.
 * Pasar `redis` para también liberar el lock entre tests del scanner.
 */
export async function cleanScannerTables(
  drizzle: DrizzleService,
  redis?: RedisService
): Promise<void> {
  await drizzle.db.delete(libraryScans);
  // Release distributed scan lock so subsequent tests can start a new scan
  if (redis) {
