...
describe('Streaming E2E', () => {
  ...
  describe('Stream Token Management', () => {
    ...
    describe('GET /api/stream-token', () => {
      it('debería obtener un token válido', () => {
        return request(app.getHttpServer())
          .get('/api/stream-token')
          .set('Authorization', `Bearer ${accessToken}`)
          .expect(200)
          .expect((res) => {
            // Token is stored as SHA-256 hash in DB, so GET returns a new one
            // (getUserToken returns null by design). Verify shape, not exact value.
            expect(res.body.token).toBeDefined();
            expect(typeof res.body.token).toBe('string');
  ...
  describe('Flujo completo de streaming', () => {
    it('debería completar flujo de token y acceso', async () => {
      ...
      // 4. Obtener token (returns a valid token — may differ from newToken
      // because getUserToken returns null by design and a fresh one is generated)
      const getRes = await request(app.getHttpServer())
        .get('/api/stream-token')
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200);

      expect(getRes.body.token).toBeDefined();
      expect(getRes.body.expiresAt).toBeDefined();

      // 5. Verificar que el token más reciente funciona
      // Use the token from GET (step 4) since it's the latest valid one.
      // getUserToken returns null → GET generates a fresh token, which may
      // differ from newToken (step 3). The latest generated token is valid.
      const latestToken = getRes.body.token;
      await request(app.getHttpServer())
        .head(`/api/tracks/${trackId}/stream?token=${latestToken}`)
        .expect((res) => {
