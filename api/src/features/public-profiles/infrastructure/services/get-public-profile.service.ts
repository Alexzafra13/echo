...
export class GetPublicProfileUseCase {
  ...
  private async getFriendshipStatus(
    ...
    const friendship = await this.drizzle.db
      ...
      .where(
        or(
          and(eq(friendships.requesterId, requesterId), eq(friendships.addresseeId, userId)),
          and(eq(friendships.requesterId, userId), eq(friendships.addresseeId, requesterId))
        )
      )
      .limit(1);
  ...
  private async getProfileStats(userId: string): Promise<ProfileStats> {
    ...
    const playsResult = await this.drizzle.db
      ...
      .from(userPlayStats)
      .where(and(eq(userPlayStats.userId, userId), eq(userPlayStats.itemType, 'track')));
  ...
  private async getListeningNow(userId: string): Promise<ListeningNow | undefined> {
    ...
    const result = await this.drizzle.db
      .select({
        ...
        currentFederationTrack: playQueues.currentFederationTrack,
      })
      .from(playQueues)
    ...
    // Federation track
    if (r.currentFederationTrack) {
      try {
        const fed = JSON.parse(r.currentFederationTrack);
        return {
          trackId: `federation-${fed.serverId}`,
          trackTitle: fed.title,
          artistName: fed.artistName,
          albumId: undefined,
          coverArtPath: fed.coverUrl ?? undefined,
        };
      } catch {
        /* no-op */
      }
  ...
  private async getTopArtists(userId: string, limit = 6): Promise<TopArtist[]> {
    const results = await this.drizzle.db
      ...
      .where(and(eq(userPlayStats.userId, userId), eq(userPlayStats.itemType, 'artist')))
      .orderBy(desc(userPlayStats.playCount))
      .limit(limit);
  ...
  private async getTopAlbums(userId: string, limit = 6): Promise<TopAlbum[]> {
    const results = await this.drizzle.db
      ...
      .where(and(eq(userPlayStats.userId, userId), eq(userPlayStats.itemType, 'album')))
      .orderBy(desc(userPlayStats.playCount))
      .limit(limit);

    // Get artist names in a single query using inArray
    const artistIds = [
      ...new Set(results.map((r) => r.artistId).filter((id): id is string => !!id)),
    ];
    ...
    if (artistIds.length > 0) {
      ...
      artistMap = Object.fromEntries(artistResults.map((a) => [a.id, a.name]));
    }
  ...
  private async getPublicPlaylists(userId: string, limit = 20): Promise<PublicPlaylist[]> {
    const results = await this.drizzle.db
      ...
      .from(playlists)
      .where(and(eq(playlists.ownerId, userId), eq(playlists.public, true)))
