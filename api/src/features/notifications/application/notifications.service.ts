...
export class NotificationsService {
  ...
  async getNotifications(
    ...
    const [result, totalResult] = await Promise.all([
      this.drizzle.db
        ...
        .limit(limit)
        .offset(offset),
      this.drizzle.db.select({ count: count() }).from(notifications).where(whereClause),
    ]);
   ...
   */
  async getPreferences(userId: string): Promise<{ notificationType: string; enabled: boolean }[]> {
   ...
   */
  async updatePreference(userId: string, type: NotificationType, enabled: boolean): Promise<void> {
    await this.drizzle.db
