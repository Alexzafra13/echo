...
export class ArtistResponseDto {
  ...
  @ApiProperty({
    required: false,
    deprecated: true,
    description: 'Legacy: use profileImageUrl instead',
  })
  @Expose()
  ...
  @ApiPropertyOptional({
    description: 'CSS background-position for hero image',
    example: '50.0% 30.0%',
  })
  @Expose()
  ...
  static fromDomain(data: ArtistData): ArtistResponseDto {
    ...
    dto.profileImageUrl = hasAnyProfileImage
      ? `/api/images/artists/${data.id}/profile${versionParam}`
      : undefined;

    // Legacy fields (deprecated but kept for backwards compatibility)
    dto.smallImageUrl = hasAnyProfileImage
      ? `/api/images/artists/${data.id}/profile${versionParam}`
      : undefined;
    dto.mediumImageUrl = hasAnyProfileImage
      ? `/api/images/artists/${data.id}/profile${versionParam}`
      : undefined;
    dto.largeImageUrl = hasAnyProfileImage
      ? `/api/images/artists/${data.id}/profile${versionParam}`
      : undefined;
    ...
    dto.backgroundPosition = data.backgroundPosition;
    dto.size = data.size;
