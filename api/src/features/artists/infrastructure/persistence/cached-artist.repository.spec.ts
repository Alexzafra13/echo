...
import { MockCacheService, createMockCacheService } from '@shared/testing/mock.types';
...
describe('CachedArtistRepository', () => {
  ...
  beforeEach(() => {
    ...
    baseRepository = {
      ...
      update: jest.fn(),
      delete: jest.fn(),
      findSimilarByGenreAndAudio: jest.fn(),
  ...
  describe('integration scenarios', () => {
    it('should handle full CRUD cycle with proper cache management', async () => {
      ...
      cacheService.get.mockResolvedValue(null);
      baseRepository.findById.mockResolvedValue(mockArtist);
      const _read1 = await cachedRepository.findById('artist-1');
      expect(cacheService.set).toHaveBeenCalled();
