  ...
  implements IArtistRepository
{
  ...
  async findById(id: string): Promise<Artist | null> {
    const result = await this.drizzle.db.select().from(artists).where(eq(artists.id, id)).limit(1);
  ...
  async findByIds(ids: string[]): Promise<Artist[]> {
    ...
    const result = await this.drizzle.db.select().from(artists).where(inArray(artists.id, ids));
  ...
  async search(name: string, skip: number, take: number): Promise<Artist[]> {
    ...
    const result = await this.drizzle.db
      .select()
      .from(artists)
      .where(and(sql`${artists.name} ILIKE ${searchPattern}`, gt(artists.songCount, 0)))
  ...
  async create(artist: Artist): Promise<Artist> {
    ...
    const result = await this.drizzle.db.insert(artists).values(persistenceData).returning();
