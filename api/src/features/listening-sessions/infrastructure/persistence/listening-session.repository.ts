...
export class DrizzleListeningSessionRepository implements IListeningSessionRepository {
  ...
  async create(session: ListeningSession): Promise<ListeningSession> {
    ...
    const result = await this.drizzle.db
      ...
      .values({
        ...
        guestsCanControl: props.guestsCanControl,
  ...
  async findActiveByHostId(hostId: string): Promise<ListeningSession | null> {
    const result = await this.drizzle.db
      .select()
      .from(listeningSessions)
      .where(and(eq(listeningSessions.hostId, hostId), eq(listeningSessions.isActive, true)))
      .limit(1);
  ...
  async findActiveByParticipantId(userId: string): Promise<ListeningSession | null> {
    const result = await this.drizzle.db
      ...
      .innerJoin(
        listeningSessions,
        eq(listeningSessionParticipants.sessionId, listeningSessions.id)
      )
      .where(
        and(eq(listeningSessionParticipants.userId, userId), eq(listeningSessions.isActive, true))
      )
      .limit(1);
  ...
  async getParticipant(sessionId: string, userId: string): Promise<SessionParticipantProps | null> {
    const result = await this.drizzle.db
      .select()
      .from(listeningSessionParticipants)
      .where(
  ...
  async getNextUnplayed(sessionId: string): Promise<QueueItemWithTrack | null> {
    const result = await this.drizzle.db
      ...
      .innerJoin(users, eq(listeningSessionQueue.addedBy, users.id))
      .where(
        and(eq(listeningSessionQueue.sessionId, sessionId), eq(listeningSessionQueue.played, false))
      )
      .orderBy(asc(listeningSessionQueue.position))
      .limit(1);
  ...
  private toDomain(row: typeof listeningSessions.$inferSelect): ListeningSession {
    return ListeningSession.fromPrimitives({
      ...
      mode: (row.mode as 'sync' | 'jukebox') || 'sync',
