...
import { CreateSessionDto, JoinSessionDto, AddToQueueDto, UpdateParticipantRoleDto } from '../dto';
...
import { ListeningSessionsGateway } from '../gateway/listening-sessions.gateway';
...
export class ListeningSessionsController {
  constructor(
    @InjectPinoLogger(ListeningSessionsController.name)
    ...
    private readonly updateSessionSettingsUseCase: UpdateSessionSettingsUseCase,
  ...
  async createSession(@Body() dto: CreateSessionDto, @Req() req: RequestWithUser) {
  ...
  async joinSession(@Body() dto: JoinSessionDto, @Req() req: RequestWithUser) {
  ...
  async getMyActiveSession(@Req() req: RequestWithUser) {
    try {
      ...
      this.logger.debug(
        { userId: req.user.id, error: error instanceof Error ? error.message : String(error) },
        'Error fetching active session, returning null'
      );
      return null;
    }
  }
  ...
  async getSession(@Param('id', ParseUUIDPipe) id: string, @Req() req: RequestWithUser) {
    return this.getSessionUseCase.execute({
      sessionId: id,
      userId: req.user.id,
    });
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Session details' })
  async getSessionByCode(@Param('code') code: string, @Req() req: RequestWithUser) {
    return this.getSessionUseCase.execute({
      inviteCode: code,
      userId: req.user.id,
    });
  ...
  async addToQueue(
    ...
    // Notificar a todos los participantes via WebSocket
    const session = await this.getSessionUseCase.execute({ sessionId, userId: req.user.id });
    this.gateway.notifyQueueUpdated(sessionId, { queue: session.queue });
    return result;
  }
  ...
  async skipTrack(@Param('id', ParseUUIDPipe) sessionId: string, @Req() req: RequestWithUser) {
    ...
    // Actividad: resetear timer
    await this.cleanupService.resetInactivityTimeout(sessionId);
    // Notificar cambio de track y cola
    const session = await this.getSessionUseCase.execute({ sessionId, userId: req.user.id });
    this.gateway.notifyTrackChanged(sessionId, {
  ...
  async updateParticipantRole(
    ...
    const result = await this.updateParticipantRoleUseCase.execute({
      sessionId,
      targetUserId: userId,
      role: dto.role,
    ...
    // Actualizar cache de sockets para que canControl() refleje el nuevo rol
    this.gateway.updateParticipantRole(sessionId, userId, dto.role as 'dj' | 'listener');
    this.gateway.notifyParticipantRoleChanged(sessionId, { userId, role: dto.role });
  ...
  async removeFromQueue(
    ...
    await this.removeFromQueueUseCase.execute({ sessionId, queueItemId, userId: req.user.id });
    const updated = await this.getSessionUseCase.execute({ sessionId, userId: req.user.id });
    this.gateway.notifyQueueUpdated(sessionId, { queue: updated.queue });
  ...
  async updateSettings(
    ...
    const result = await this.updateSessionSettingsUseCase.execute({
      sessionId,
      userId: req.user.id,
      guestsCanControl: dto.guestsCanControl,
    });

    // Actualizar cache de sockets para que canControl() refleje el cambio
    if (dto.guestsCanControl !== undefined) {
      this.gateway.updateSessionSetting(sessionId, dto.guestsCanControl);
    }
  ...
  async leaveSession(@Param('id', ParseUUIDPipe) sessionId: string, @Req() req: RequestWithUser) {
  ...
  async endSession(@Param('id', ParseUUIDPipe) sessionId: string, @Req() req: RequestWithUser) {
    ...
    // Notificar fin de sesion a todos
