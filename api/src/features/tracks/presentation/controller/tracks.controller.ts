...
export class TracksController {
  ...
  async getShuffledTracks(
    ...
    const dto = ShuffledTracksResponseDto.fromDomain(result);
    await this.videoEnrichment.enrichTracksWithVideoIds(dto.data);
    return dto;
  }
  ...
  async getDjShuffledTracks(
    ...
    const dto = DjShuffledTracksResponseDto.fromDomain(result);
    await this.videoEnrichment.enrichTracksWithVideoIds(dto.data);
    return dto;
  }
  ...
  @ApiOperation({
    ...
    description:
      'Retorna el análisis DJ (BPM, Key, Camelot, Energy, Danceability) de un track específico',
  })
  @ApiParam({
    name: 'id',
    type: String,
  ...
  @ApiOperation({
    ...
    description:
      'Busca tracks por título (mínimo 2 caracteres). La búsqueda es case-insensitive y busca coincidencias parciales',
  })
  @ApiParam({
    name: 'query',
    type: String,
