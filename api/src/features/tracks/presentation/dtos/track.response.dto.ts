...
export class TrackResponseDto {
  ...
  @ApiPropertyOptional({
    description: 'UUID del artista del álbum',
  ...
  @ApiProperty({
    ...
    example: '/music/beatles/abbey-road/01-come-together.mp3',
  })
  @Expose()
  path!: string;
  ...
  @ApiPropertyOptional({
    description: 'Seconds where outro/silence begins (for smart crossfade)',
    example: 234.5,
  })
  @Expose()
  outroStart?: number;
  ...
  @ApiPropertyOptional({
    description: 'Associated music video ID',
  ...
  static fromDomain(track: TrackData): TrackResponseDto {
    ...
    const data =
      typeof trackAsEntity.toPrimitives === 'function'
    ...
    // BPM
    dto.bpm = data.bpm;
    // videoId is set externally by VideoEnrichmentService
    // Missing file status
