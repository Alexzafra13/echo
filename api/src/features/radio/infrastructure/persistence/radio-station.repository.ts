...
export class DrizzleRadioStationRepository implements IRadioStationRepository {
  ...
  async save(station: RadioStation): Promise<RadioStation> {
    ...
    if (existing.length > 0) {
      ...
      // Insert new record
      const result = await this.drizzle.db.insert(radioStations).values(data).returning();
   ...
   */
  async findByStationUuid(userId: string, stationUuid: string): Promise<RadioStation | null> {
    const result = await this.drizzle.db
      .select()
      .from(radioStations)
      .where(and(eq(radioStations.userId, userId), eq(radioStations.stationUuid, stationUuid)))
      .limit(1);
  ...
  async findByUserId(userId: string): Promise<RadioStation[]> {
    const result = await this.drizzle.db
      .select()
      .from(radioStations)
      .where(and(eq(radioStations.userId, userId), eq(radioStations.isFavorite, true)))
  ...
  async delete(id: string): Promise<void> {
    await this.drizzle.db.delete(radioStations).where(eq(radioStations.id, id));
  }
