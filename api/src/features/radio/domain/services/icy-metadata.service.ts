...
// Max time a stream can be active without any subscribers before cleanup (10 minutes)
const ZOMBIE_STREAM_TIMEOUT_MS = 10 * 60 * 1000;
// How often to check for zombie streams (2 minutes)
const ZOMBIE_CHECK_INTERVAL_MS = 2 * 60 * 1000;
...
export class IcyMetadataService implements OnModuleInit, OnModuleDestroy {
  ...
  private zombieCleanupInterval: ReturnType<typeof setInterval> | null = null;
  ...
  onModuleInit() {
    ...
    // Periodically clean up zombie streams (no subscribers for too long)
    this.zombieCleanupInterval = setInterval(() => {
      this.cleanupZombieStreams();
    }, ZOMBIE_CHECK_INTERVAL_MS);

    this.logger.info('IcyMetadataService initialized with global error safety net');
  }

  private cleanupZombieStreams(): void {
    const now = Date.now();
    for (const [stationUuid] of this.activeStreams) {
      const listeners = this.streamListeners.get(stationUuid);
      const lastActivity = this.streamLastActivity.get(stationUuid) ?? 0;
      const hasNoListeners = !listeners || listeners.size === 0;
      const isStale = now - lastActivity > ZOMBIE_STREAM_TIMEOUT_MS;

      if (hasNoListeners && isStale) {
        this.logger.warn(
          `Cleaning up zombie ICY stream for ${stationUuid} (no listeners for ${Math.round((now - lastActivity) / 1000)}s)`
        );
        this.closeStream(stationUuid);
        this.streamLastActivity.delete(stationUuid);
      }
    }
  }
  ...
  subscribe(stationUuid: string, streamUrl: string): EventEmitter {
    ...
    this.streamLastActivity.set(stationUuid, Date.now());
