...
export class AdminLibraryController {
  ...
  async getLibrary() {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        readable = true;
        const counts = await this.countMediaFiles(savedPath);
        fileCount = counts.audioCount;
  ...
  async updateLibrary(@Body() dto: UpdateLibraryPathDto) {
    ...
    // Guardar en settings y sincronizar con FilesystemService
    await this.settingsService.set(LIBRARY_PATH_KEY, normalizedPath);
    this.filesystemService.addAllowedRoot(normalizedPath);
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && !entry.name.startsWith('.')) {
          ...
          try {
            ...
            hasMusic = subEntries.some((f) => /\.(mp3|flac|m4a|ogg|wav|aac|opus|wma)$/i.test(f));
          } catch {
            readable = false;
          }
  ...
  private async countMediaFiles(
    dirPath: string,
    depth = 0
  ): Promise<{ audioCount: number; videoCount: number }> {
    if (depth > 5) return { audioCount: 0, videoCount: 0 };

    const audioExtensions = /\.(mp3|flac|m4a|ogg|wav|aac|opus|wma|ape)$/i;
    const videoExtensions = /\.(mp4|webm|m4v)$/i;
    let audioCount = 0;
    let videoCount = 0;
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isFile()) {
          if (audioExtensions.test(entry.name)) audioCount++;
          else if (videoExtensions.test(entry.name)) videoCount++;
        } else if (entry.isDirectory() && depth < 3) {
          const sub = await this.countMediaFiles(fullPath, depth + 1);
