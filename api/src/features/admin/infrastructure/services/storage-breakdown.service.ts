...
export class StorageBreakdownService {
  ...
  async get(): Promise<StorageBreakdown> {
    ...
    const [
      ...
      metadataSizeResult,
      avatarSizeResult,
      radioFaviconSizeResult,
    ] = await Promise.all([
      this.drizzle.db.select({ sum: sum(tracks.size) }).from(tracks),
      this.drizzle.db.select({ sum: sum(musicVideos.size) }).from(musicVideos),
    ...
    const videos = Number(videosSizeResult[0]?.sum || 0);
    ...
    const breakdown: StorageBreakdown = {
      ...
      avatars,
      radioFavicons,
      total: music + videos + metadata + avatars + radioFavicons,
    };
