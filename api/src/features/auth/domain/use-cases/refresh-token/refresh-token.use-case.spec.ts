...
describe('RefreshTokenUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new RefreshTokenUseCase(mockUserRepository, mockTokenService, mockTokenBlacklist);
  });
  ...
  describe('execute', () => {
    ...
    it('debería invalidar el refresh token antiguo al generar nuevos tokens', async () => {
      // Arrange
      mockTokenService.verifyRefreshToken.mockResolvedValue({
        userId: 'user-123',
        username: 'juan',
      ...
      // Act
      await useCase.execute({ refreshToken: 'old_refresh_token' });

      // Assert
      expect(mockTokenBlacklist.add).toHaveBeenCalledWith('old-token-jti', 1700000000);
    });
    ...
    it('debería lanzar error si refresh token está expirado', async () => {
      // Arrange
      mockTokenService.verifyRefreshToken.mockRejectedValue(new Error('Token expired'));
    ...
    it('debería lanzar error si refresh token está vacío', async () => {
      // Arrange
      mockTokenService.verifyRefreshToken.mockRejectedValue(new Error('Token required'));
