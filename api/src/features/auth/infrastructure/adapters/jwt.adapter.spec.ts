...
describe('JwtAdapter', () => {
  ...
  describe('generateAccessToken', () => {
    ...
    it('should include a jti (UUID) in the access token payload', async () => {
      const user = createMockUser({ id: 'user-123', username: 'juan' });
      mockJwtService.sign.mockReturnValue('access_token_123');

      await adapter.generateAccessToken(user);
  ...
  describe('generateRefreshToken', () => {
    ...
    it('should include a jti (UUID) in the refresh token payload', async () => {
      const user = createMockUser({ id: 'user-123', username: 'juan' });
      mockJwtService.sign.mockReturnValue('refresh_token_123');

      await adapter.generateRefreshToken(user);
  ...
  describe('verifyAccessToken', () => {
    ...
    it('debería lanzar error si token es inválido', async () => {
      ...
      // Act & Assert
      await expect(adapter.verifyAccessToken('invalid_token')).rejects.toThrow();
    });
  });
});
