...
describe('UserMapper', () => {
  ...
  describe('toPersistence', () => {
    it('should convert domain User to persistence format (camelCase)', () => {
      const user = UserMapper.toDomain(mockDbUser as unknown as UserDb);
      const persistence = UserMapper.toPersistence(user);

      expect(persistence.id).toBe('user-1');
      expect(persistence.username).toBe('testuser');
      expect(persistence.passwordHash).toBe('$2b$12$hash');
      expect(persistence.isActive).toBe(true);
      expect(persistence.isAdmin).toBe(false);
      expect(persistence.theme).toBe('dark');
      expect(persistence.language).toBe('es');
      expect(persistence.isPublicProfile).toBe(true);
      expect(persistence.showTopTracks).toBe(true);
    });
    ...
    it('should convert undefined optional fields to null', () => {
      ...
      expect(persistence.avatarPath).toBeNull();
  ...
  describe('roundtrip', () => {
    it('should preserve data through toDomain -> toPersistence', () => {
      ...
      expect(persistence.passwordHash).toBe(mockDbUser.passwordHash);
    });
  });
});
