...
describe('ExploreService', () => {
  ...
  const setupQueryMocks = (mainResult: Record<string, unknown>[], countResult: number = 0) => {
    const createChain = (): DrizzleChainMock => {
      ...
      // Make chain thenable - single query returns results with totalCount
      chain.then = (resolve: (value: unknown) => unknown) => {
        const resultsWithCount = mainResult.map((r) => ({ ...r, totalCount: countResult }));
        return Promise.resolve(resultsWithCount).then(resolve);
      };
