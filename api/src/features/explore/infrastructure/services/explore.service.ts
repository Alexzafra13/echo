...
export class ExploreService {
  ...
  async getUnplayedAlbums(
    ...
    // Get albums NOT in the played list + total count in a single query
    ...
    return {
      albums: result.map(({ totalCount: _, ...album }) => album),
      total,
    };
  }
  ...
  async getForgottenAlbums(
    ...
    // Albums played by user but not recently + total count in single query
    const result = await this.db
      .select({
        id: albums.id,
        name: albums.name,
    ...
    return {
      albums: result.map(({ lastPlayedAt: _lastPlayedAt, totalCount: _, ...album }) => album),
      total,
    };
  }
  ...
  async getHiddenGems(userId: string, limit: number = 30): Promise<ExploreTrack[]> {
    ...
    const result = await this.db
      ...
      .where(
        and(
          sql`${tracks.artistId} IN (${sql.join(
            topArtistIds.map((id: string) => sql`${id}::uuid`),
            sql`, `
          )})`,
