...
describe('DjAnalysisQueueService', () => {
  ...
  // ─── formatDuration (extraído a @shared/utils) ────────────────────

  describe('formatDuration', () => {
    // formatDuration fue extraído a @shared/utils/format-duration.util.ts
    // Los tests unitarios de la función están en format-duration.util.spec.ts
    // Aquí verificamos que el servicio sigue usándola correctamente (integración implícita)
    it('should be available as shared util', async () => {
      const { formatDuration } = await import('@shared/utils');
