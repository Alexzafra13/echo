...
import { DjEnergyCalibrationService } from './dj-energy-calibration.service';
...
export class DjAnalysisQueueService implements OnModuleInit {
  ...
  constructor(
    ...
    private readonly energyCalibration: DjEnergyCalibrationService,
  ...
  private async resumePendingAnalyses(): Promise<void> {
    ...
    try {
      // Solo resetear records "analyzing" que llevan más de 3 minutos (stuck).
      // Los que están procesándose activamente tienen updatedAt reciente.
      const staleThreshold = new Date(Date.now() - 3 * 60 * 1000);
      await this.drizzle.db
        .update(djAnalysis)
        .set({ status: 'pending', updatedAt: new Date() })
        .where(
          sql`${djAnalysis.status} = 'analyzing' AND ${djAnalysis.updatedAt} < ${staleThreshold}`
        );
  ...
  /**
   * Incrementa progreso y verifica si la sesión terminó.
   * Centraliza la lógica duplicada en los 3 paths de processAnalysisJob.
   */
  private advanceProgress(processingTimeMs?: number): void {
    ...
    if (this.processedInSession >= this.totalToProcess) {
      ...
      this.energyCalibration.recalibrate().catch((err) => {
        this.logger.warn(
          { error: err instanceof Error ? err.message : 'Unknown' },
          'Calibración de energía falló (no crítico)'
        );
      });
    }
  }
