...
export class ArtistPlaylistService {
  ...
  async getPaginated(
    ...
    // Cargar todos los IDs (max 50 — ligero) para contar el total.
    // Solo se generan playlists para la página solicitada.
    const allTopArtists = await this.playTrackingRepo.getUserTopArtists(userId, 50);
    const total = allTopArtists.length;

    const paginatedArtistIds = allTopArtists.slice(skip, skip + take).map((a) => a.artistId);
    ...
    const playlists = await this.buildPlaylists(
      userId,
      paginatedArtistIds,
      artistMap,
      tracksByArtist
  ...
  private async loadArtistData(
    ...
    const allTracks = await this.drizzle.db
      ...
      .from(tracks)
      .where(and(inArray(tracks.id, trackIds), inArray(tracks.artistId, artistIds)));
