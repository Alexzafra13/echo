...
import { trackGenres, genres } from '@infrastructure/database/schema';
...
export class PlaylistShuffleService {
  ...
  async calculateMetadata(
    ...
    const lastMonth = relevantHistory.filter(
      (h) => h.playedAt >= oneMonthAgo && h.playedAt < oneWeekAgo
    ).length;
    const lastYear = relevantHistory.filter(
      (h) => h.playedAt >= oneYearAgo && h.playedAt < oneMonthAgo
    ).length;
    const older = relevantHistory.filter((h) => h.playedAt < oneYearAgo).length;

    // Calculate average score
    const avgScore =
      tracks.length > 0 ? tracks.reduce((sum, t) => sum + t.totalScore, 0) / tracks.length : 0;

    // Get top artists
    const artistIds = tracks
      .map((t) => trackMap.get(t.trackId)?.artistId)
      .filter((id) => id) as string[];
    const artistCounts = artistIds.reduce(
      (acc, id) => {
        acc[id] = (acc[id] || 0) + 1;
        return acc;
      },
    ...
    // Get top genres from track_genres join
    const topGenres = await this.getTopGenresForTracks(trackIds);
  ...
  private async getTopGenresForTracks(trackIds: string[]): Promise<string[]> {
    if (trackIds.length === 0) return [];

    const results = await this.drizzle.db
      .select({
        name: genres.name,
        trackCount: count(trackGenres.trackId),
      })
      .from(trackGenres)
      .innerJoin(genres, eq(trackGenres.genreId, genres.id))
      .where(inArray(trackGenres.trackId, trackIds))
      .groupBy(genres.name)
      .orderBy(sql`count(${trackGenres.trackId}) DESC`)
      .limit(5);
