...
export class DrizzleSocialRepository implements ISocialRepository {
  ...
  async getListeningFriends(userId: string): Promise<ListeningUser[]> {
    ...
    return results.map((r) => {
      // Parse federation track info if available
      let federationTrack: {
        title: string;
        artistName: string;
        albumName: string;
      ...
      if (!r.trackId && r.currentFederationTrack) {
        try {
          federationTrack = JSON.parse(r.currentFederationTrack);
        } catch {
          /* no-op */
        }
      }
      ...
      return {
        ...
        currentTrack: r.trackId
          ...
          : federationTrack
            ? {
                id: `federation-${federationTrack.serverId}`,
                title: federationTrack.title,
                artistName: federationTrack.artistName,
                albumName: federationTrack.albumName,
                albumId: '',
                coverPath: federationTrack.coverUrl || null,
              }
            : null,
  ...
  async getFriendsActivity(userId: string, limit: number = 20): Promise<ActivityItem[]> {
    ...
    const playlistResults = await this.drizzle.db
      ...
      .from(playlists)
      .where(and(inArray(playlists.ownerId, friendIds), eq(playlists.public, true)))
    ...
    const results = await this.drizzle.db
      ...
      .where(
        and(
          ...
          or(ilike(users.username, searchPattern), ilike(users.name, searchPattern))
        )
      )
      .limit(limit);
    ...
    const friendshipResults = await this.drizzle.db
      ...
      .where(
        or(
          ...
          and(eq(friendships.addresseeId, currentUserId), inArray(friendships.requesterId, userIds))
        )
      );
