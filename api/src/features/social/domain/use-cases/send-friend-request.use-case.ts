...
export class SendFriendRequestUseCase {
  constructor(
    @InjectPinoLogger(SendFriendRequestUseCase.name)
  ...
  async execute(requesterId: string, addresseeId: string): Promise<Friendship> {
    ...
    const existing = await this.socialRepository.getFriendshipBetweenUsers(
      requesterId,
      addresseeId
    );
    ...
    if (existing) {
      ...
      if (existing.status === 'pending') {
        ...
        if (existing.addresseeId === requesterId) {
          const accepted = await this.socialRepository.acceptFriendRequest(
            existing.id,
            requesterId
          );
          // Notify both users about mutual acceptance
          this.notifyAccepted(requesterId, addresseeId).catch((e) => {
    ...
    this.notifyRequestReceived(requesterId, addresseeId, friendship.id).catch((e) => {
      this.logger.warn(`No se pudo notificar solicitud de amistad: ${(e as Error).message}`);
    });
  ...
  private async getUserDisplayName(userId: string): Promise<string | null> {
    return this.socialRepository.getUserDisplayName(userId);
  }
