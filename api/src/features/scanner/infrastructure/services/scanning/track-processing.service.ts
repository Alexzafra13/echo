  ...
  async processFile(
    ...
    try {
      ...
      const trackData = {
        ...
        comment:
          typeof metadata.comment === 'object' && metadata.comment?.text
      ...
      if (existingTrack) {
        ...
        // Si el album cambió, limpiar el viejo si quedó sin tracks
        if (albumChanged && oldAlbumId) {
          await this.libraryCleanup.cleanupOrphanedAlbum(oldAlbumId);
        }
