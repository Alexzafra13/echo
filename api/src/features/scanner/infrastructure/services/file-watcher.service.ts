...
import { SUPPORTED_AUDIO_EXTENSIONS } from './supported-extensions';
...
export class FileWatcherService implements OnModuleInit, OnModuleDestroy {
  ...
  private handleReady(path: string): void {
    ...
    this.logger.info(`Supported extensions: ${SUPPORTED_AUDIO_EXTENSIONS.join(', ')}`);
    this.logger.info(`Debounce: ${this.DEBOUNCE_MS / 1000}s after last change`);
  }

  private isSupportedFile(filePath: string): boolean {
    const ext = filePath.toLowerCase();
    return SUPPORTED_AUDIO_EXTENSIONS.some((supported) => ext.endsWith(supported));
  }
  ...
  async resume(): Promise<void> {
    ...
    if (musicPath) {
      await this.startWatching(musicPath);
      this.logger.info('File watcher reanudado');
    }
  }
}
