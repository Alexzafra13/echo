...
export class ScanProcessorService implements OnModuleInit {
  ...
  onModuleInit() {
    // Dos colas separadas: 'library-scan' para scans completos (con lock distribuido
    // y pause/cancel) y 'scanner' para incrementales del file watcher (ligeros, sin lock).
  ...
  async enqueueScan(scanId: string, options?: EnqueueScanOptions): Promise<void> {
    ...
    const lockTTL = 3600; // 1 hora máximo — se libera al terminar el scan

    const acquired = await this.redis.setNX(lockKey, scanId, lockTTL);
    if (!acquired) {
  ...
  private async processScanning(data: ScanJobData): Promise<void> {
    ...
    try {
      ...
      this.scannerGateway.emitCompleted({
        ...
        artistsCreated: tracker.artistsCreated,
        coversExtracted: tracker.coversExtracted,
        videosFound: tracker.videosFound,
        errors: tracker.errors,
        duration,
        timestamp: new Date().toISOString(),
      });
      ...
      this.notificationsService
        .notifyAll(
          'scan_completed',
          'Escaneo completado',
          tracker.errors > 0 ? `${summary} (${tracker.errors} errores)` : summary,
          {
            scanId,
            duration,
            totalFiles: tracker.totalFiles,
    ...
    try {
      ...
      if (videoFiles.length > 0) {
        ...
        for (const videoPath of videoFiles) {
          ...
          if ((result === 'added' || result === 'updated') && tracker) tracker.videosFound++;
        }
      }
