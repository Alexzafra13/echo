...
describe('StartScanUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('should propagate error if enqueueScan detects concurrent scan (lock)', async () => {
      // Arrange
      const mockScan = createMockScan();
      scannerRepository.create.mockResolvedValue(mockScan);
      scanProcessor.enqueueScan.mockRejectedValue(new ScannerError('SCAN_ALREADY_RUNNING'));
