...
import { VideoProcessingService } from './infrastructure/services/scanning/video-processing.service';

/**
 * NOTA DE ARQUITECTURA: Los servicios del scanner acceden directamente a
 * DrizzleService en vez de usar repositorios de dominio (ITrackRepository, etc.).
 *
 * Razón: El scanner es un proceso batch que ejecuta 6-7 operaciones de DB
 * por archivo (check existencia, find/create artist, find/create album,
 * insert/update track, stats, géneros). Los mappers domain↔persistence
 * añadirían overhead significativo en un loop de miles de archivos sin
 * beneficio real — no hay lógica de negocio compleja, solo ETL.
 *
 * Este es un trade-off deliberado: rendimiento batch > pureza arquitectónica.
 */
@Module({
  imports: [
    QueueModule,
