...
export class ScannerController {
  ...
  async startScan(@Body() dto: StartScanRequestDto): Promise<StartScanResponseDto> {
  ...
  @ApiResponse({ status: 401, description: 'No autenticado' })
  @ApiResponse({ status: 403, description: 'No autorizado (solo admins)' })
  async getScansHistory(@Query() query: ScansHistoryQueryDto): Promise<ScansHistoryResponseDto> {
  ...
  async getMissingFiles() {
    try {
      ...
      this.logger.error(
        `Error getting missing files: ${(error as Error).message}`,
  ...
  async purgeMissingFiles() {
    try {
      ...
      this.logger.error(
        `Error purging missing files: ${(error as Error).message}`,
        (error as Error).stack
      );
      throw error;
    }
    ...
    try {
      ...
      if (!result.trackDeleted) {
        return {
          success: false,
          message:
            'No se pudo eliminar el track. Puede que no exista o no esté marcado como desaparecido.',
        };
      }
      ...
      this.logger.error(
        `Error deleting missing track ${id}: ${(error as Error).message}`,
        (error as Error).stack
      );
      throw error;
    }
