...
import { ConflictError, ValidationError, ExternalApiError, TimeoutError } from '@shared/errors';
import {
...
export class RemoteServerService {
  ...
  async connectToServer(
    ...
    if (existing) {
      throw new ConflictError('Already connected to this server');
    }

    if (requestMutual && !localServerUrl) {
      throw new ValidationError('Local server URL is required for mutual federation');
    }
  ...
  async getRemoteAlbum(
    ...
    try {
      ...
      const [browseResult, exportResult, djResult] = await Promise.allSettled([
        ...
        }>(server, `/api/federation/albums/${albumId}/export`),
        this.makeAuthenticatedRequest<{
          tracks: Array<{
            trackId: string;
            status: string;
        ...
        }>(server, `/api/federation/albums/${albumId}/dj-analysis`),
      ]);
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new TimeoutError(RemoteServerService.REQUEST_TIMEOUT, `Request to ${baseUrl}`);
      }
      ...
      throw new ExternalApiError('Federation', 502, errorMessage);
    } finally {
      clearTimeout(timeoutId);
    }
  }
