...
import { encryptString, decryptString, isEncrypted } from '@shared/utils';
...
export class FederationRepository implements IFederationRepository {
  constructor(
    ...
    private readonly securitySecrets: SecuritySecretsService
  ) {}

  // Encryption helpers for authToken at rest
  private encryptAuthToken(token: string): string {
    return encryptString(token, this.securitySecrets.jwtSecret);
  }

  private decryptAuthToken(token: string): string {
    // Migration support: if the token is not encrypted (legacy), return as-is
    if (!isEncrypted(token)) return token;
    return decryptString(token, this.securitySecrets.jwtSecret) ?? token;
  }

  private decryptConnectedServer(server: ConnectedServer): ConnectedServer {
    return { ...server, authToken: this.decryptAuthToken(server.authToken) };
  }
  ...
  private mapToFederationAccessToken(
    record: typeof federationAccessTokens.$inferSelect
  ): FederationAccessToken {
    return {
      ...record,
  ...
  async createConnectedServer(data: NewConnectedServer): Promise<ConnectedServer> {
    const [server] = await this.drizzle.db
      ...
      .values({ ...data, authToken: this.encryptAuthToken(data.authToken) })
      .returning();
    return this.decryptConnectedServer(server);
  }
  ...
  async findConnectedServerById(id: string): Promise<ConnectedServer | null> {
    const [server] = await this.drizzle.db
      ...
      .limit(1);
    return server ? this.decryptConnectedServer(server) : null;
  }

  async findConnectedServersByUserId(userId: string): Promise<ConnectedServer[]> {
    const servers = await this.drizzle.db
      .select()
      .from(connectedServers)
      .where(eq(connectedServers.userId, userId));
    return servers.map((s) => this.decryptConnectedServer(s));
  }
  ...
  async findConnectedServerByUrl(userId: string, baseUrl: string): Promise<ConnectedServer | null> {
    const [server] = await this.drizzle.db
      .select()
      .from(connectedServers)
      .where(and(eq(connectedServers.userId, userId), eq(connectedServers.baseUrl, baseUrl)))
      .limit(1);
    return server ? this.decryptConnectedServer(server) : null;
  }
  ...
  async updateConnectedServer(
    ...
    if (updateData.authToken) {
      updateData.authToken = this.encryptAuthToken(updateData.authToken);
    }
    const [server] = await this.drizzle.db
  ...
  async createFederationToken(data: NewFederationToken): Promise<FederationToken> {
    const [token] = await this.drizzle.db.insert(federationTokens).values(data).returning();
    return token;
  }
  ...
  async createFederationAccessToken(
    ...
    const [token] = await this.drizzle.db.insert(federationAccessTokens).values(data).returning();
  ...
  async findFederationAccessTokenByToken(token: string): Promise<FederationAccessToken | null> {
    const [result] = await this.drizzle.db
      ...
      .where(
        and(eq(federationAccessTokens.token, token), eq(federationAccessTokens.isActive, true))
      )
      .limit(1);
  ...
  async createAlbumImport(data: NewAlbumImportQueue): Promise<AlbumImportQueue> {
    const [importItem] = await this.drizzle.db.insert(albumImportQueue).values(data).returning();
