...
export class FederationPublicController {
  ...
  async streamTrack(
    ...
    // Validar path contra whitelist de directorios permitidos
    let safePath: string;
    try {
    ...
    if (!fs.existsSync(safePath)) {
      res.status(HttpStatus.NOT_FOUND).send({ error: 'Track file not found' });
      return;
    }

    const fileSize = track.size ?? fs.statSync(safePath).size;
    ...
    if (range) {
      ...
      // Usar FilesystemService para validación de path (defensa en profundidad)
      const stream = this.filesystemService.createReadStream(track.path, start, end);
      stream.pipe(res.raw);
    } else {
      ...
      const stream = this.filesystemService.createReadStream(track.path);
      stream.pipe(res.raw);
    }
  }
