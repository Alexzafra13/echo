...
describe('StreamTokenService', () => {
  ...
  describe('generateToken', () => {
    it('debería generar un token de 64 caracteres', async () => {
      ...
      // Mock para invalidateCacheForUser (select)
    ...
    it('debería eliminar tokens anteriores del usuario e invalidar cache', async () => {
      // Arrange
      const userId = 'user-123';
      const mockWhere = jest.fn().mockResolvedValue(undefined);
      mockDrizzle.db.delete.mockReturnValue({ where: mockWhere });
      // Mock para invalidateCacheForUser: select token del usuario actual
      mockDrizzle.db.select.mockReturnValue({
        from: jest.fn().mockReturnValue({
          where: jest.fn().mockReturnValue({
            limit: jest.fn().mockResolvedValue([]),
  ...
  describe('validateToken', () => {
    it('debería retornar userId para token válido (cache miss)', async () => {
      // Arrange
    ...
    it('debería retornar userId desde cache (cache hit)', async () => {
      // Arrange
      const token = 'valid-token';
      const userId = 'user-123';

      mockCache.get.mockResolvedValue(userId);
    ...
    it('debería disparar update lastUsedAt fire-and-forget para token válido', async () => {
      // Arrange
      const futureDate = new Date(Date.now() + 1000 * 60 * 60);
      const mockSetWhere = jest.fn().mockReturnValue({ catch: jest.fn() });
  ...
  describe('getUserToken', () => {
    it('debería retornar null siempre (token hasheado no es recuperable)', async () => {
      // El token en DB es un hash — no se puede devolver al cliente
  ...
  describe('revokeToken', () => {
    it('debería eliminar el token del usuario e invalidar cache específica', async () => {
      // Arrange
      const userId = 'user-123';
      const tokenHash = 'abc123hash';
      const mockDeleteWhere = jest.fn().mockResolvedValue(undefined);
      mockDrizzle.db.delete.mockReturnValue({ where: mockDeleteWhere });
      // Mock invalidateCacheForUser: select encuentra token existente
      mockDrizzle.db.select.mockReturnValue({
        from: jest.fn().mockReturnValue({
          where: jest.fn().mockReturnValue({
            limit: jest.fn().mockResolvedValue([{ token: tokenHash }]),
          }),
        }),
      });
      ...
      // Assert
      expect(mockDrizzle.db.delete).toHaveBeenCalled();
      expect(mockCache.del).toHaveBeenCalledWith(`stream-token:${tokenHash}`);
      // Ya no usa delPattern
      expect(mockCache.delPattern).not.toHaveBeenCalled();
    });

    it('debería funcionar si no hay token previo en cache', async () => {
      // Arrange
      const userId = 'user-123';
      const mockDeleteWhere = jest.fn().mockResolvedValue(undefined);
      mockDrizzle.db.delete.mockReturnValue({ where: mockDeleteWhere });
