...
export class StreamingController implements OnModuleDestroy {
  ...
  private createManagedStream(
    ...
    // Usa FilesystemService para doble validación de path (defensa en profundidad)
    const stream = this.filesystemService.createReadStream(filePath, options?.start, options?.end);

    this.activeStreams.add(stream);
    this.activeStreamsTracker.increment();

    const cleanup = (): void => {
      if (this.activeStreams.delete(stream)) {
        this.activeStreamsTracker.decrement();
      }
    };
