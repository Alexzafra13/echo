...
export class AlbumsController {
  constructor(
    ...
    private readonly getUserTopPlayedAlbumsUseCase: GetUserTopPlayedAlbumsUseCase,
  ...
  async getRecentAlbums(@Query('take') take?: string): Promise<AlbumResponseDto[]> {
  ...
  @ApiOperation({
    ...
    description:
      'Retorna los álbumes con más reproducciones basado en estadísticas reales de reproducción del usuario',
  })
  @ApiQuery({
    name: 'take',
    required: false,
  ...
  async getTopPlayedAlbums(@Query('take') take?: string): Promise<AlbumResponseDto[]> {
    const { take: takeNum } = parsePaginationParams(undefined, take, {
      defaultTake: 10,
      maxTake: 50,
    });
  ...
  @ApiOperation({
    summary: 'Obtener álbumes más reproducidos del usuario',
    description: 'Retorna los álbumes más reproducidos por el usuario autenticado',
  })
  @ApiQuery({
    name: 'take',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de álbumes más reproducidos del usuario obtenida exitosamente',
    type: [AlbumResponseDto],
  })
  @ApiResponse({
    status: 401,
  ...
  async getUserTopPlayedAlbums(
    ...
    const result = await this.getUserTopPlayedAlbumsUseCase.execute({
      userId: user.id,
      take: takeNum,
    });
  ...
  @ApiOperation({
    ...
    description:
      'Retorna álbumes ordenados por nombre (ignora artículos como "The", "A", etc. y acentos)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Retorna álbumes ordenados por nombre de artista (ignora artículos como "The", "A", etc. y acentos)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Retorna álbumes del historial de reproducción del usuario, ordenados por última reproducción',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Retorna el álbum destacado para mostrar en la sección hero (generalmente el más reproducido o más reciente)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Retorna todas las canciones (tracks) de un álbum específico, ordenadas por disco y número de pista',
  })
  @ApiParam({
    name: 'id',
    type: String,
  ...
  async getAlbumTracks(
    ...
    await this.videoEnrichment.enrichTracksWithVideoIds(data);
  ...
  @ApiOperation({
    ...
    description:
      'Busca álbumes por nombre (mínimo 2 caracteres). La búsqueda es case-insensitive y busca coincidencias parciales',
  })
  @ApiParam({
    name: 'query',
    type: String,
