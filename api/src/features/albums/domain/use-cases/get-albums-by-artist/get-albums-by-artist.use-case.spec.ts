...
import { GetAlbumsByArtistUseCase } from './get-albums-by-artist.use-case';
import { ALBUM_REPOSITORY, IAlbumRepository } from '../../ports/album-repository.port';
import { Album } from '../../entities/album.entity';

describe('GetAlbumsByArtistUseCase', () => {
  let useCase: GetAlbumsByArtistUseCase;
  let albumRepository: jest.Mocked<IAlbumRepository>;

  const createMockAlbum = (id: string, name: string, artistName: string) =>
    Album.reconstruct({
      id,
      name,
      artistId: 'artist-1',
  ...
  const mockAlbums = [
    createMockAlbum('album-1', 'Abbey Road', 'The Beatles'),
    createMockAlbum('album-2', 'Let It Be', 'The Beatles'),
    createMockAlbum('album-3', 'Dark Side of the Moon', 'Pink Floyd'),
  ];
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<GetAlbumsByArtistUseCase>(GetAlbumsByArtistUseCase);
    albumRepository = module.get(ALBUM_REPOSITORY);
  });

  describe('execute', () => {
    it('debería retornar álbumes ordenados por nombre de artista con paginación', async () => {
      // Arrange
      (albumRepository.findByArtistName as jest.Mock).mockResolvedValue(mockAlbums);
      ...
      // Assert
      expect(albumRepository.findByArtistName).toHaveBeenCalledWith(0, 20);
    ...
    it('debería calcular skip correctamente para página 2', async () => {
      ...
      // Assert
      expect(albumRepository.findByArtistName).toHaveBeenCalledWith(20, 20);
    });
    ...
    it('debería calcular skip correctamente para página 5', async () => {
      ...
      // Assert
      expect(albumRepository.findByArtistName).toHaveBeenCalledWith(40, 10);
    });
    ...
    it('debería limitar limit a máximo 100', async () => {
      ...
      // Assert
      expect(albumRepository.findByArtistName).toHaveBeenCalledWith(0, 100);
    ...
    it('debería retornar array vacío si no hay álbumes', async () => {
      // Arrange
      (albumRepository.findByArtistName as jest.Mock).mockResolvedValue([]);
    ...
    it('debería ejecutar findByArtistName y count en paralelo', async () => {
      // Arrange
      let findByArtistNameResolved = false;
      let countResolved = false;

      (albumRepository.findByArtistName as jest.Mock).mockImplementation(() => {
        return new Promise((resolve) => {
          setTimeout(() => {
            findByArtistNameResolved = true;
            resolve(mockAlbums);
          }, 10);
        });
      });
      ...
      expect(findByArtistNameResolved).toBe(true);
      expect(countResolved).toBe(true);
      expect(endTime - startTime).toBeLessThan(30);
    });

    it('debería preservar el orden por nombre de artista', async () => {
      // Arrange
      const orderedAlbums = [
        createMockAlbum('album-3', 'Dark Side of the Moon', 'Pink Floyd'),
        createMockAlbum('album-1', 'Abbey Road', 'The Beatles'),
      ...
      (albumRepository.findByArtistName as jest.Mock).mockResolvedValue(orderedAlbums);
      (albumRepository.count as jest.Mock).mockResolvedValue(3);

      // Act
      const result = await useCase.execute({ page: 1, limit: 20 });

      // Assert
      expect(result.albums[0].artistName).toBe('Pink Floyd');
      expect(result.albums[1].artistName).toBe('The Beatles');
      expect(result.albums[2].artistName).toBe('The Beatles');
    });
  });
});
