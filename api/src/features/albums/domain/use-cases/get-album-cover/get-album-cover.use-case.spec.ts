...
describe('GetAlbumCoverUseCase', () => {
  ...
  let mockAlbumRepository: { findCoverPaths: jest.Mock } & Partial<IAlbumRepository>;
  ...
  // Helper to set the repository result
  function setDbResult(
    row: { coverArtPath: string | null; externalCoverPath: string | null } | null
  ) {
    mockAlbumRepository.findCoverPaths.mockResolvedValue(row);
  }
