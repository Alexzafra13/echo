...
import { eq, desc, or, inArray, count, sql } from 'drizzle-orm';
  ...
  implements IAlbumRepository
{
  ...
  async findMostPlayedByUser(userId: string, take: number): Promise<Album[]> {
    const topAlbumIds = await this.drizzle.db.execute<{
      ...
      SELECT item_id as "albumId", SUM(play_count) as "totalPlayCount"
      FROM user_play_stats
      WHERE item_type = 'album' AND user_id = ${userId}
      GROUP BY item_id
      ORDER BY "totalPlayCount" DESC
      LIMIT ${take}
    `);
  ...
  async create(album: Album): Promise<Album> {
    ...
    const result = await this.drizzle.db.insert(albums).values(persistenceData).returning();
  ...
  async findCoverPaths(
    albumId: string
  ): Promise<{ coverArtPath: string | null; externalCoverPath: string | null } | null> {
    const result = await this.drizzle.db
      .select({
