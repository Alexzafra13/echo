...
import { UserPlaySummary } from '../../domain/entities/play-event.entity';
  ...
  beforeEach(async () => {
    module = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: TrackEnricherService,
          useValue: { enrichTopTracks: jest.fn().mockResolvedValue([]) },
        },
      ...
      const enriched = [
        {
          trackId: 'track-1',
          playCount: 10,
          weightedPlayCount: 8.5,
          track: { id: 'track-1', title: 'Test' },
        },
      ];

      getUserTopTracksUseCase.execute.mockResolvedValue(mockTopTracks);
      const trackEnricher = module.get(TrackEnricherService);
      (trackEnricher.enrichTopTracks as jest.Mock).mockResolvedValue(enriched);

      const req = { user: mockUser } as unknown as RequestWithUser;
      const result = await controller.getTopTracks(req, 50, undefined, undefined);
