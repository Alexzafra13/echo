...
describe('UpdatePlaybackStateUseCase', () => {
  ...
  describe('execute', () => {
    it('should update playback state when playing with a track', async () => {
      ...
      expect(repository.updatePlaybackState).toHaveBeenCalledWith('user-1', true, 'track-1', null);
    ...
    it('should set isPlaying to false when undefined', async () => {
      ...
      expect(repository.updatePlaybackState).toHaveBeenCalledWith('user-1', false, null, null);
      expect(listeningNowService.emitUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          userId: 'user-1',
          isPlaying: false,
    ...
    it('should set trackId to null when playing but no trackId provided', async () => {
      ...
      expect(repository.updatePlaybackState).toHaveBeenCalledWith('user-1', true, null, null);
    });
    ...
    it('should call logger.debug with playback state', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        { userId: 'user-1', isPlaying: true, currentTrackId: 'track-1', hasFederationTrack: false },
    ...
    it('should propagate repository errors', async () => {
      (repository.updatePlaybackState as jest.Mock).mockRejectedValue(new Error('Database error'));
