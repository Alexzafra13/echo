...
export class DrizzlePlayTrackingRepository implements IPlayTrackingRepository {
  ...
  async updatePlayStats(
    ...
    // Transacción: actualizar stats de track + album + artist atómicamente.
    // Sin transacción, un fallo parcial deja stats inconsistentes entre entidades.
    await this.drizzle.db.transaction(async (tx) => {
      await this.updateItemStatsInTx(tx, userId, trackId, 'track', weightedPlay, completionRate);
      ...
      if (track[0].albumId) {
        await this.updateItemStatsInTx(
          tx,
          userId,
          track[0].albumId,
          'album',
  ...
  private async updateItemStatsInTx(
  ...
  async getUserPlayStats(userId: string, itemType?: string): Promise<PlayStats[]> {
    const conditions = [eq(userPlayStats.userId, userId)];
    if (itemType) {
      conditions.push(eq(userPlayStats.itemType, itemType));
    }
  ...
  async getUserPlaySummary(userId: string, days: number = 30): Promise<UserPlaySummary> {
    ...
    const dateFilter = and(eq(playHistory.userId, userId), gte(playHistory.playedAt, since));

    // 3 queries en paralelo: stats globales, plays recientes, y distribución por contexto
    const [statsResult, recentPlays, contextRows] = await Promise.all([
      this.drizzle.db
        .select({
          totalPlays: count(),
          avgCompletionRate: avg(playHistory.completionRate),
          // Contar skips sobre TODOS los plays del periodo (no solo los 50 recientes)
          totalSkips: sql<number>`count(*) FILTER (WHERE ${playHistory.skipped} = true)`,
        })
        .from(playHistory)
        .where(dateFilter),
      this.drizzle.db
        ...
        .orderBy(desc(playHistory.playedAt))
        .limit(50),
      // Distribución por contexto sobre TODOS los plays (no solo 50)
      this.drizzle.db
        .select({
          context: playHistory.playContext,
          count: count(),
        })
        .from(playHistory)
        .where(dateFilter)
        .groupBy(playHistory.playContext),
    ]);

    const playsByContext = contextRows.reduce(
      (acc, row) => {
        acc[row.context as PlayContext] = Number(row.count);
        return acc;
      },
    ...
    return {
      totalPlays: statsResult[0]?.totalPlays ?? 0,
      totalSkips: Number(statsResult[0]?.totalSkips) || 0,
      avgCompletionRate: Number(statsResult[0]?.avgCompletionRate) || 0,
      topContext,
      playsByContext,
      recentPlays: PlayTrackingMapper.toPlayEventDomainArray(recentPlays),
    };
  }
  ...
  async updatePlaybackState(
    ...
    // Validate that currentTrackId is a valid UUID
    ...
    // Store federation track info as JSON when playing a federated track (mutually exclusive with local trackId)
    const federationJson =
      !trackIdToStore && federationTrack ? JSON.stringify(federationTrack) : null;

    // Use upsert: insert if not exists, update if exists
    await this.drizzle.db.execute(sql`
      INSERT INTO play_queue (user_id, current_track_id, current_federation_track, is_playing, updated_at)
      VALUES (${userId}, ${trackIdToStore}, ${federationJson}, ${isPlaying}, NOW())
      ON CONFLICT (user_id)
      DO UPDATE SET
        current_track_id = ${trackIdToStore},
        current_federation_track = ${federationJson},
