...
export class CachedPlayTrackingRepository implements IPlayTrackingRepository {
  ...
  /**
   * Invalidate the most visible/impactful caches related to a user.
   * Called on write operations (recordPlay, recordSkip, updatePlayStats).
   *
   * Only invalidates: recent, user-stats, top-tracks.
   * top-albums, top-artists, summary and listening-time refresh via natural TTL (10-15 min).
   */
  private async invalidateUserCaches(userId: string): Promise<void> {
    ...
    this.logger.debug(
      { userId },
      'User play tracking cache invalidated (recent, stats, top-tracks)'
    );
  }
}
