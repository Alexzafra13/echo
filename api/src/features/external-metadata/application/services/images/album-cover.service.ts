...
export class AlbumCoverService {
  ...
  async getAlbumCover(albumId: string): Promise<CachedImageResult> {
    ...
    const result: CachedImageResult = await this.resolveCoverPath(coverPath, source);
  ...
  async getAlbumCoverResized(
    ...
    CachedImageResult | { filePath: string; mimeType: string; size: number; resized: true }
  > {
    ...
    const resized = await this.resize.getResized(original.filePath, albumId, size, preferWebP);
