...
export class AdminSettingsController {
  ...
  async updateSetting(@Param('key') key: string, @Body() dto: UpdateSettingDto) {
    ...
    // Redact sensitive values (API keys, secrets) from logs
    const isSensitive =
      key.includes('api_key') || key.includes('secret') || key.includes('password');
    const safeValue = isSensitive ? '***' : dto.value;
    const safeOldValue = isSensitive ? '***' : oldValue;

    this.logger.info(
      isCreating
        ? `Setting ${key} created with value "${safeValue}"`
        : `Setting ${key} updated from "${safeOldValue}" to "${safeValue}"`
    );
  ...
  async validateApiKey(@Body() dto: ValidateApiKeyDto) {
    ...
    try {
      ...
      this.logger.warn(`API key validation failed for ${dto.service}: ${(error as Error).message}`);
      return {
        valid: false,
        service: dto.service,
