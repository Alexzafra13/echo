...
export class MusicBrainzAgent implements IMusicBrainzSearch {
  ...
  readonly capabilities = ['IMusicBrainzSearch'];
  private readonly baseUrl = 'https://musicbrainz.org/ws/2';
  private readonly userAgent = 'Echo-Music-Server/1.0.0 (https://github.com/Alexzafra13/echo)';

  constructor(
    @InjectPinoLogger(MusicBrainzAgent.name)
  ...
  async searchArtist(artistName: string, limit = 5): Promise<MusicBrainzArtistMatch[]> {
    try {
      ...
      this.logger.debug(`Found ${matches.length} artist matches for: ${artistName}`);

      return matches;
    } catch (error) {
      this.logger.error(`Error searching artist "${artistName}": ${(error as Error).message}`);
      return [];
    }
  }
  ...
  async searchAlbum(
    ...
    try {
      ...
      // Parse and map results
      const matches: MusicBrainzAlbumMatch[] = (data['release-groups'] || []).map(
        (rg: MBReleaseGroupResponse) => ({
          mbid: rg.id,
          title: rg.title,
      ...
      this.logger.debug(`Found ${matches.length} album matches for: ${albumTitle}`);

      return matches;
    } catch (error) {
      this.logger.error(`Error searching album "${albumTitle}": ${(error as Error).message}`);
      return [];
    }
  }
  ...
  async getArtistByMbid(mbid: string): Promise<MusicBrainzArtistMatch | null> {
    try {
      ...
      this.logger.error(`Error fetching artist MBID ${mbid}: ${(error as Error).message}`);
      return null;
    }
  }
  ...
  async getAlbumByMbid(mbid: string): Promise<MusicBrainzAlbumMatch | null> {
    try {
      ...
      this.logger.error(`Error fetching album MBID ${mbid}: ${(error as Error).message}`);
      return null;
    }
  }
  ...
  async searchRecording(
    ...
    try {
      ...
      // Parse and map results
      const matches: MusicBrainzRecordingMatch[] = (data.recordings || []).map(
        (rec: MBRecordingResponse) => {
