...
import { FanartImage, FanartImageArray } from './types';
...
export class FanartTvAgent implements IArtistImageRetriever, IAlbumCoverRetriever, OnModuleInit {
  ...
  readonly capabilities = ['IArtistImageRetriever', 'IAlbumCoverRetriever'];
  ...
  async loadSettings(): Promise<void> {
    ...
    const dbEnabled =
      (await this.settingsService.getBoolean('metadata.fanart.enabled', true)) &&
   ...
   */
  async getArtistImages(mbid: string | null, name: string): Promise<ArtistImages | null> {
    if (!mbid) {
  ...
  async getAllAlbumCoverVariants(
    ...
    try {
      ...
      this.logger.debug(
        `Fetching ALL Fanart.tv album covers: ${artistName} - ${albumName} (artist: ${artistMbid}, album: ${albumMbid})`
      );
  ...
  async getAlbumCoverByArtist(
    ...
    try {
      ...
      const url = `${this.baseUrl}/${artistMbid}`;
      this.logger.debug(
        `Fetching Fanart.tv data for artist to find album: ${artistName} - ${albumName} (artist: ${artistMbid}, album: ${albumMbid})`
      );
      ...
      this.logger.debug(`Retrieved album cover from Fanart.tv for: ${artistName} - ${albumName}`);
