...
export class LastfmAgent implements IArtistBioRetriever, IArtistImageRetriever, OnModuleInit {
  ...
  readonly capabilities = ['IArtistBioRetriever', 'IArtistImageRetriever'];
  ...
  async loadSettings(): Promise<void> {
    ...
    const dbEnabled =
      (await this.settingsService.getBoolean('metadata.lastfm.enabled', true)) &&
    ...
    if (!this.apiKey) {
      ...
      this.logger.info(
        `Last.fm agent initialized (enabled: ${this.enabled}, priority: ${this.priority})`
      );
    }
  }
  ...
  async getArtistBio(mbid: string | null, name: string): Promise<ArtistBio | null> {
    ...
    for (const lang of this.bioLanguages) {
      try {
        ...
        return new ArtistBio(content, summary || null, artistInfo.url || null, this.name);
      } catch (error) {
        this.logger.debug(
          `Error fetching ${lang} biography for ${name}: ${(error as Error).message}`
        );
  ...
  private async getArtistInfo(
    ...
    this.logger.debug(
      `Fetching Last.fm artist info: ${mbid || name}${lang ? ` (lang: ${lang})` : ''}`
    );
