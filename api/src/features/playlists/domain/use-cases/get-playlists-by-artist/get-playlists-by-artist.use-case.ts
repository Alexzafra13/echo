...
export class GetPlaylistsByArtistUseCase {
  ...
  async execute(input: GetPlaylistsByArtistInput): Promise<GetPlaylistsByArtistOutput> {
    ...
    const playlists = await this.playlistRepository.findPublicByArtistId(
      input.artistId,
      skip,
      take,
      input.userId
    );
    const total = await this.playlistRepository.countPublicByArtistId(input.artistId, input.userId);
