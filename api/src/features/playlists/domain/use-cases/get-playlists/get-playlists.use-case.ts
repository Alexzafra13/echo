...
import { GetPlaylistsInput, GetPlaylistsOutput } from './get-playlists.dto';
...
export class GetPlaylistsUseCase {
  ...
  async execute(input: GetPlaylistsInput): Promise<GetPlaylistsOutput> {
    ...
    if (input.search) {
      playlists = await this.playlistRepository.search(input.search, skip, take);
      total = playlists.length < take ? skip + playlists.length : skip + take + 1;
      ...
      throw new ValidationError('Must specify ownerId, publicOnly, or search filter');
    }
