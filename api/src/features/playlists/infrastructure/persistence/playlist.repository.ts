...
export class DrizzlePlaylistRepository implements IPlaylistRepository {
  ...
  async addTrackWithAutoOrder(playlistId: string, trackId: string): Promise<PlaylistTrack> {
    // Use a transaction with row-level locking to prevent concurrent inserts
    // from calculating the same nextOrder (READ COMMITTED doesn't prevent this).
    // SELECT FOR UPDATE on the playlist row serializes concurrent addTrack calls.
    return await this.drizzle.db.transaction(async (tx) => {
      // 0. Lock the playlist row to serialize concurrent inserts
  ...
  async getPlaylistTracks(
    ...
    skip?: number,
    take?: number
  ): Promise<TrackWithPlaylistOrder[]> {
  ...
  async recalculateMetadata(
    playlistId: string
  ): Promise<{ duration: number; size: number; songCount: number }> {
    const result = await this.drizzle.db
      .select({
        duration: sql<number>`COALESCE(SUM(${tracks.duration}), 0)::int`,
        size: sql<number>`COALESCE(SUM(${tracks.size}), 0)::bigint`,
    ...
    const stats = result[0] || { duration: 0, size: 0, songCount: 0 };
    ...
    return { duration: stats.duration, size: Number(stats.size), songCount: stats.songCount };
  }
  ...
  async countPublicByArtistId(artistId: string, userId?: string): Promise<number> {
    ...
    const result = await this.drizzle.db
      .select({ count: sql<number>`COUNT(DISTINCT ${playlists.id})::int` })
      .from(playlists)
      .innerJoin(playlistTracks, eq(playlists.id, playlistTracks.playlistId))
      .innerJoin(tracks, eq(playlistTracks.trackId, tracks.id))
      .where(and(visibilityFilter, eq(tracks.artistId, artistId)));
