...
export class PlaylistsController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Buscar playlists por nombre',
  })
  @ApiResponse({
  ...
  async getPlaylists(
    ...
    const result = await this.getPlaylistsUseCase.execute({
      ownerId: search ? undefined : publicOnly ? undefined : userId,
  ...
  async getPlaylistTracks(
    ...
    const dto = PlaylistTracksResponseDto.fromDomain(result);
    await this.videoEnrichment.enrichTracksWithVideoIds(dto.tracks);
    return dto;
  }
