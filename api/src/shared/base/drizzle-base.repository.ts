 ...
 * ```
 */
export abstract class DrizzleBaseRepository<TDomain, TPersistence = Record<string, unknown>> {
   ...
   */
  private static readonly baseLogger = new Logger('DrizzleBaseRepository');
  ...
  async delete(id: string): Promise<boolean> {
    ...
    const result = await this.drizzle.db.delete(this.table).where(eq(idColumn, id)).returning();
   ...
   */
  protected toPrimitives(entity: Partial<TDomain>): TPersistence | Partial<TDomain> {
    if (entity && 'toPrimitives' in entity && typeof entity.toPrimitives === 'function') {
