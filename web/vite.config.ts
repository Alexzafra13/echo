  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          {
            // Image metadata endpoints return JSON (not binary images) and must
            // always be fresh so tag-based cache busting works after image changes.
            // This rule MUST come before the CacheFirst image rule below.
            urlPattern: /\/api\/images\/.*\/(all|exists|metadata)/,
          ...
          {
            // Cache album covers and artist image binaries
            ...
            // Tag-based URLs (?tag=hash) ensure new images get new cache entries.
