 ...
 */
// Cache para evitar recalcular el color de la misma imagen
const colorCache = new Map<string, string>();
const MAX_CACHE_SIZE = 200;

function cacheResult(key: string, value: string): string {
  if (colorCache.size >= MAX_CACHE_SIZE) {
    // Eliminar la entrada mas antigua (primera del Map)
...
export async function extractDominantColor(imageSrc: string): Promise<string> {
  ...
  const result = await extractDominantColorUncached(imageSrc);
  return cacheResult(imageSrc, result);
}

async function extractDominantColorUncached(imageSrc: string): Promise<string> {
