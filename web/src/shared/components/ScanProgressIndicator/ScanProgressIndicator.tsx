...
export function ScanProgressIndicator() {
  ...
  const { status, progress, filesScanned, totalFiles, tracksCreated } = currentScan;
  ...
  // Format file count
  const fileCount = totalFiles > 0 ? `${filesScanned}/${totalFiles}` : `${filesScanned}`;

  return (
    <div
      className={`${styles.container} ${styles[`container--${statusClass}`]}`}
      title={t('scan.scanningLibrary', { filesScanned, tracksCreated })}
    >
      <div className={`${styles.iconWrapper} ${styles[`iconWrapper--${statusClass}`]}`}>
        {getIcon()}
      </div>
      ...
      {scans.length > 1 && <span className={styles.badge}>+{scans.length - 1}</span>}
    </div>
  );
}
