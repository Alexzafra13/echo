  ...
  useEffect(() => {
    ...
    if (currentButton) {
      ...
      if (isMobile && listRef.current) {
        ...
        const scrollTarget =
          listRef.current.scrollLeft + (btnLeft - listLeft) - listWidth / 2 + btnWidth / 2;
        ...
        {pages.map((page, index) => {
          if (page === 'ellipsis') {
            return (
              <li
                key={`ellipsis-${index}`}
                className={`${styles.pagination__item} ${styles.pagination__gap}`}
              >
          ...
          return (
            <li key={pageNumber} className={styles.pagination__item}>
              <button
                ...
                aria-label={t('common.page', { current: pageNumber, total: totalPages })}
              >
                {pageNumber}
              </button>
            </li>
