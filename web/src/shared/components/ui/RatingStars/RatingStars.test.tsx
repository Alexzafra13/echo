...
describe('RatingStars', () => {
  ...
  describe('rendering', () => {
    ...
    it('should render with initial rating', () => {
      render(<RatingStars itemId={mockItemId} itemType={mockItemType} initialRating={3} />);
  ...
  describe('interactions', () => {
    it('should call setRating when clicking a star', async () => {
      ...
      await waitFor(() => {
        expect(interactionsService.setRating).toHaveBeenCalledWith(mockItemId, mockItemType, 4);
      });
    });
    ...
    it('should call removeRating when clicking the same star twice', async () => {
      ...
      const { rerender: _rerender } = render(
        <RatingStars itemId={mockItemId} itemType={mockItemType} initialRating={3} />
      );
      ...
      await waitFor(() => {
        expect(interactionsService.removeRating).toHaveBeenCalledWith(mockItemId, mockItemType);
      });
    });
  ...
  describe('readOnly mode', () => {
    it('should disable all buttons in readOnly mode', () => {
      render(<RatingStars itemId={mockItemId} itemType={mockItemType} readOnly />);
    ...
    it('should have readOnly class', () => {
      const { container } = render(
        <RatingStars itemId={mockItemId} itemType={mockItemType} readOnly />
      );
  ...
  describe('loading state', () => {
    it('should show loading class when making API call', async () => {
      ...
      const { container } = render(<RatingStars itemId={mockItemId} itemType={mockItemType} />);
  ...
  describe('initialRating from parent', () => {
    it('should use initialRating prop without fetching from API', () => {
      render(<RatingStars itemId={mockItemId} itemType={mockItemType} initialRating={4} />);

      // No API call should be made — rating comes from parent
      expect(interactionsService.setRating).not.toHaveBeenCalled();
      // Component renders without errors
      expect(screen.getAllByRole('button')).toHaveLength(5);
    });

    it('should update when initialRating prop changes', () => {
      const { rerender } = render(
        <RatingStars itemId={mockItemId} itemType={mockItemType} initialRating={2} />
      );

      rerender(<RatingStars itemId={mockItemId} itemType={mockItemType} initialRating={5} />);
  ...
  describe('size customization', () => {
    it('should apply custom size to stars', () => {
      render(<RatingStars itemId={mockItemId} itemType={mockItemType} size={24} />);
  ...
  describe('error handling', () => {
    it('should handle setRating error', async () => {
      ...
      vi.mocked(interactionsService.setRating).mockRejectedValue(new Error('Network error'));
      ...
      await waitFor(() => {
        expect(consoleError).toHaveBeenCalledWith('Error updating rating:', expect.any(Error));
      });
