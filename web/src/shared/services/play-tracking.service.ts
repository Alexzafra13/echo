...
export async function recordSkip(data: RecordSkipData): Promise<void> {
  try {
    const completionRate = data.totalDuration > 0 ? data.timeListened / data.totalDuration : 0;
...
export async function getTopTracks(
  ...
  const response = await apiClient.get(
    `/play-tracking/top-tracks?limit=${limit}&timeRange=${timeRange}`
  );
  return response.data;
}
...
/**
 * Federation track info for "listening now" feature
 */
export interface FederationTrackData {
  title: string;
  artistName: string;
  albumName: string;
...
export interface PlaybackStateData {
  ...
  federationTrack?: FederationTrackData | null;
}
