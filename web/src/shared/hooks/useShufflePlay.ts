...
import {
  ...
  type DjShuffledTracksResponse,
} from '@features/home/services/tracks.service';
...
export function useShufflePlay(): UseShufflePlayReturn {
    ...
    try {
      ...
      while (newTracks.length < BATCH_SIZE && attempts < maxAttempts) {
        ...
        if (!response.hasMore) {
          ...
          if (
            state.seenTrackIds.size + newTracks.length >= state.totalTracks &&
            newTracks.length < BATCH_SIZE
          ) {
  ...
  useEffect(() => {
    if (
      shuffleRef.current.seed &&
      hasMore &&
      !shuffleRef.current.loading &&
