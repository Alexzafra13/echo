  ...
  useEffect(() => {
    ...
    const findScrollContainer = (): HTMLElement | null => {
      ...
      if (parent) {
        ...
        // Strategy 4: Buscar en hermanos del abuelo (header dentro de wrapper)
        const grandparent = parent.parentElement;
        if (grandparent) {
          const gpChildren = Array.from(grandparent.children) as HTMLElement[];
          const gpScrollable = gpChildren.find((child) => {
            if (child === parent) return false;
            const s = window.getComputedStyle(child);
            return s.overflowY === 'auto' || s.overflowY === 'scroll';
          });
          if (gpScrollable) return gpScrollable;
        }
      }
