...
export function useArtistMetadataSync(artistId?: string) {
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener('metadata:artist:images:updated', handleArtistImagesUpdated);

    return () => {
      eventSource.removeEventListener('metadata:artist:images:updated', handleArtistImagesUpdated);
    };
