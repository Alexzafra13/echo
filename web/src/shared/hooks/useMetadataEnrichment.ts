...
export function useMetadataEnrichment(_token: string | null, isAdmin: boolean) {
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener('metadata:enrichment:started', handleStarted);
    eventSource.addEventListener('metadata:enrichment:progress', handleProgress);
    eventSource.addEventListener('metadata:enrichment:completed', handleCompleted);
    eventSource.addEventListener('metadata:enrichment:error', handleError);

    return () => {
      eventSource.removeEventListener('open', onOpen);
      eventSource.removeEventListener('error', onError);
      eventSource.removeEventListener('metadata:enrichment:started', handleStarted);
      eventSource.removeEventListener('metadata:enrichment:progress', handleProgress);
      eventSource.removeEventListener('metadata:enrichment:completed', handleCompleted);
      eventSource.removeEventListener('metadata:enrichment:error', handleError);
    };
  }, [eventSource, isAdmin]);
