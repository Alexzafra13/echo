...
export function useAlbumMetadataSync(albumId?: string, artistId?: string) {
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener('metadata:album:cover:updated', handleAlbumCoverUpdated);

    return () => {
      eventSource.removeEventListener('metadata:album:cover:updated', handleAlbumCoverUpdated);
    };
