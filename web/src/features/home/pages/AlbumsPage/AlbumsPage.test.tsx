...
vi.mock('../../components', () => ({
  ...
  AlbumGrid: ({
    ...
    <div data-testid="album-grid">
      ...
      {albums.map((album) => (
        <div key={album.id} data-testid="album-card">
          {album.title}
        </div>
      ))}
    </div>
...
vi.mock('@features/federation', () => ({
  ...
  SharedAlbumGrid: ({ albums }: { albums: Array<{ id: string; title: string }> }) => (
    <div data-testid="shared-album-grid">
      {albums.map((album) => (
        <div key={album.id} data-testid="shared-album-card">
          {album.title}
        </div>
      ))}
    </div>
...
describe('AlbumsPage', () => {
  ...
  describe('Search', () => {
    ...
    it('should filter albums by search query', () => {
      ...
      // Search is now server-side via useAlbumSearch (debounced).
      // With mock returning null for search data, filteredAlbums falls back to allAlbums.
      // Typing a query updates the input value; results depend on server response.
      fireEvent.change(searchInput, { target: { value: 'Album One' } });

      // All albums still shown because search is server-side and mock returns null
      expect(screen.getAllByTestId('album-card')).toHaveLength(3);
  ...
  describe('Shared Library', () => {
    ...
    it('should show different placeholder text in search', () => {
      ...
      expect(
        screen.getByPlaceholderText('Buscar en bibliotecas compartidas...')
      ).toBeInTheDocument();
    });
