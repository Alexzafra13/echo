  ...
  const renderOptions = (
    ...
    <>
      {onAddToPlaylist && (
        ...
        >
          <ListPlus size={18} />
          <span>{t('tracks.addToPlaylist')}</span>
        </button>
      )}
      ...
      {canAddToSession && (
        ...
        >
          <Radio size={18} />
          <span>{t('tracks.addToSession')}</span>
        </button>
      )}
      ...
      {onRemoveFromPlaylist && (
        ...
        >
          <Trash2 size={18} />
          <span>{t('tracks.removeFromPlaylist')}</span>
        </button>
      )}
    ...
    <>
      <div className={styles.trackOptionsMenu}>
        <button
          ...
          aria-label={t('tracks.trackOptions')}
          aria-expanded={isMobile ? sheetOpen : dropdownOpen}
          title={t('tracks.moreOptions')}
        >
      ...
      {!isMobile && dropdownOpen && effectivePosition && (
        <Portal>
          ...
          >
            {renderOptions(
              (e, cb) => handleDropdownOptionClick(e, cb, track),
              false /* no rating on desktop */
            )}
          </div>
        </Portal>
      )}
        ...
        <Portal>
          ...
          >
            ...
            <div ref={dragScrollRef} className={styles.trackOptionsMenu__sheetContent}>
              {renderOptions(
                handleSheetOptionClick,
                !hideRating /* show rating on mobile unless explicitly hidden */
              )}
            </div>
          </div>
        </Portal>
