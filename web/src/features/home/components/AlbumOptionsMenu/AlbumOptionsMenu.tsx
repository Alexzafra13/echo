...
import { useAuth, useDropdownMenu, useSheetDragToClose, useIsMobile } from '@shared/hooks';
    ...
    <>
      ...
      {onAddToPlaylist && (
        ...
        >
          ...
          <span>{t('albums.addToPlaylist')}</span>
        </button>
      )}
      ...
      {user?.isAdmin && onChangeCover && (
        ...
        >
          ...
          <span>{t('albums.changeCover')}</span>
        </button>
      )}
    ...
    <>
      <div className={styles.albumOptionsMenu}>
        <button
          ...
          aria-label={t('albums.options')}
          aria-expanded={isMobile ? sheetOpen : isOpen}
          title={t('albums.moreOptions')}
        >
