...
vi.mock('@features/playlists/components', () => ({
  PlaylistCoverMosaic: () => <div data-testid="playlist-mosaic">Cover</div>,
}));
...
describe('PublicProfilePage', () => {
  ...
  describe('error state', () => {
    it('should show error state when user not found', () => {
      ...
      expect(
        screen.getByText('El usuario que buscas no existe o ha sido eliminado.')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('private profile', () => {
    it('should show private profile message', () => {
      ...
      expect(
        screen.getByText('Este usuario ha configurado su perfil como privado.')
      ).toBeInTheDocument();
    });
  ...
  describe('public profile rendering', () => {
    ...
    it('should render avatar placeholder when no avatar', () => {
      ...
      // Avatar renders as img with default placeholder when no avatarUrl
      const avatar = screen.getByAltText('Test User');
      expect(avatar).toBeInTheDocument();
      expect(avatar.getAttribute('src')).toContain('avatar-default');
    });
  ...
  describe('empty state', () => {
    it('should show empty state when no content', () => {
      ...
      expect(
        screen.getByText('Este usuario aún no tiene actividad para mostrar.')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('helper functions', () => {
    ...
    it('should use default avatar when no name and no avatar', () => {
      vi.mocked(usePublicProfile).mockReturnValue({
        data: {
          ...mockProfile,
    ...
    it('should use default avatar when no name, username or avatar', () => {
      vi.mocked(usePublicProfile).mockReturnValue({
        data: {
          ...mockProfile,
      ...
      // Avatar renders with default image
      const avatars = document.querySelectorAll('img[src*="avatar-default"]');
