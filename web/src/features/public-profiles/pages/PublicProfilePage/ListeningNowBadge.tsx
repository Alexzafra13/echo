...
export function ListeningNowBadge({ listeningNow }: ListeningNowBadgeProps) {
  ...
  const className = `${styles.publicProfilePage__listeningNow} ${
    isVisible
      ? styles['publicProfilePage__listeningNow--visible']
      : styles['publicProfilePage__listeningNow--hidden']
  }`;

  return (
    <Link href={displayData.albumId ? `/album/${displayData.albumId}` : '#'} className={className}>
