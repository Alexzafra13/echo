...
vi.mock('../../components', () => ({
  ArtistOptionsMenu: ({ onChangeProfile }: { onChangeProfile: () => void }) => (
    <button data-testid="artist-options" onClick={onChangeProfile}>
      Options
    </button>
  ),
}));
...
// Mock music videos
vi.mock('@features/music-videos/components/MusicVideosSection/MusicVideosSection', () => ({
  MusicVideosSection: () => null,
}));

vi.mock('@features/music-videos/services/music-videos.service', () => ({
  musicVideosService: { getByArtistId: vi.fn().mockResolvedValue([]) },
}));
...
const mockArtist = {
  ...
  songCount: 50,
  biography:
    'This is a test biography for the artist. It contains information about their career and musical journey.',
...
describe('ArtistDetailPage', () => {
  beforeEach(() => {
    ...
    vi.mocked(playerModule.usePlayback).mockReturnValue({
      play: vi.fn(),
      pause: vi.fn(),
      currentTrack: null,
      isPlaying: false,
    } as unknown as ReturnType<typeof playerModule.usePlayback>);
  });
  ...
  describe('albums section', () => {
    ...
    it('should show empty albums message when no albums', () => {
      ...
      expect(screen.getByText('No hay álbumes disponibles')).toBeInTheDocument();
    });
  });
  ...
  describe('related artists section', () => {
    it('should render related artists', () => {
      ...
      expect(screen.getByText('Artistas relacionados')).toBeInTheDocument();
  ...
  describe('biography section', () => {
    ...
    it('should show placeholder when no biography', () => {
      ...
      expect(screen.getByText('No hay biografía disponible')).toBeInTheDocument();
    });
