 ...
 */
export function BiographySection({ biography, biographySource }: BiographySectionProps) {
  ...
  return (
    <section className={styles.artistDetailPage__biography}>
      <div className={styles.artistDetailPage__biographyHeader}>
        ...
        <h2 className={styles.artistDetailPage__sectionTitle}>{t('artists.biography')}</h2>
      </div>

      {hasBiography ? (
        <div className={styles.artistDetailPage__biographyContent}>
          <div
            className={`${styles.artistDetailPage__biographyText} ${
              !isBioExpanded && isLongBiography
                ? styles.artistDetailPage__biographyText__collapsed
                : ''
            }`}
          >
          ...
          {isLongBiography && (
            ...
            >
              {isBioExpanded ? t('artists.readLess') : t('artists.readMore')}
            </button>
          )}

          {biographySource && (
            <div className={styles.artistDetailPage__biographySource}>
              {t('artists.biographySource', { source: getSourceName(biographySource) })}
            </div>
          )}
        </div>
      ) : (
        <p className={styles.artistDetailPage__biographyPlaceholder}>{t('artists.noBiography')}</p>
      )}
    </section>
  );
}
