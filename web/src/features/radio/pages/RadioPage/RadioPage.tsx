...
export default function RadioPage() {
  ...
  useDocumentTitle(t('radio.pageTitle'));
  ...
  const availableGenres = useMemo(() => {
    ...
    if (favoriteStations.length > 0) {
      genres.splice(2, 0, {
        id: 'favorites',
        label: t('radio.favoritesLabel', { count: favoriteStations.length }),
        icon: '💙',
      });
    }

    return genres;
  }, [favoriteStations.length, t]);
  ...
  const handleResultSelect = useCallback(
    (station: RadioStation | RadioBrowserStation) => {
      playRadio(station);
  ...
  const handleToggleFavorite = useCallback(
    async (station: RadioBrowserStation | RadioStation) => {
      try {
        const stationUuid = 'stationuuid' in station ? station.stationuuid : station.stationUuid;

        const isInFavorites = favoriteStations.some((fav) => fav.stationUuid === stationUuid);

        if (isInFavorites) {
          const favoriteStation = favoriteStations.find((fav) => fav.stationUuid === stationUuid);
  ...
  const isStationPlaying = useCallback(
    (station: RadioBrowserStation | RadioStation) => {
      ...
      const stationUuid = 'stationuuid' in station ? station.stationuuid : station.stationUuid;
      const currentUuid =
        'stationuuid' in currentRadioStation
  ...
  const selectedCountryName = useMemo(() => {
    ...
    return country?.name || t('radio.yourCountry');
  }, [selectedCountry, allCountries, t]);
      ...
      <main className={styles.radioPage__main}>
        ...
        <div ref={contentRef} className={styles.radioPage__content}>
          <div className={styles.radioPage__pageHeader}>
            <h1 className={styles.radioPage__pageTitle}>{t('radio.pageTitle')}</h1>
            <p className={styles.radioPage__pageSubtitle}>{t('radio.subtitle')}</p>
          </div>

          <div className={styles.radioPage__filters}>
            <button className={styles.radioPage__genreButton} onClick={genreModal.open}>
              <Music2 size={20} />
              <span>{t('radio.genreLabel', { label: activeFilterLabel })}</span>
          ...
          <div className={styles.radioPage__section}>
            <h2 className={styles.radioPage__title}>
              ...
              {isFavoritesFilter
                ? t('radio.myFavorites')
                : isAllCountries
                  ? t('radio.topWorldStations')
                  : t('radio.stationsFrom', { country: selectedCountryName })}
              ...
              {stations.length > 0 && (
                ...
                >
                  ({t('radio.stationCount', { count: stations.length })})
                </span>
              )}
            </h2>
            ...
            {stationsError ? (
              ...
              <div className={styles.radioPage__empty}>
                <Radio size={48} />
                <p>{t('radio.noStationsFound')}</p>
                <p className={styles.radioPage__emptyHint}>{t('radio.noStationsHint')}</p>
              </div>
            )}
          </div>
        </div>
