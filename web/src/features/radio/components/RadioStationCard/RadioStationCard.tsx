...
export const RadioStationCard = memo(function RadioStationCard({
  ...
  const handleSelectPreview = useCallback(
    ...
    [stationUuid, savePreviewMutation, name]
  );
    ...
    <>
      ...
      >
        <div className={styles.radioCard__coverContainer}>
          <img
            src="/images/radio/echo_radio_dark.svg"
            alt=""
            className={`${styles.radioCard__fallback} ${styles['radioCard__fallback--dark']}`}
          />
          <img
            src="/images/radio/echo_radio_light.svg"
            alt=""
            className={`${styles.radioCard__fallback} ${styles['radioCard__fallback--light']}`}
          />
          {displayFavicon && (
            <img
              src={displayFavicon}
            ...
            {isAdmin && stationUuid && (
              <div className={styles.radioCard__adminActions}>
                <button
                  ...
                  disabled={isLoading}
                  aria-label={t('radio.uploadFavicon')}
                  title={t('radio.uploadFavicon')}
                >
                  <ImagePlus size={14} />
                </button>
                <button
                  ...
                  disabled={isLoading}
                  aria-label={t('radio.searchFavicon')}
                  title={t('radio.searchFavicon')}
                >
                  <Wand2 size={14} />
                </button>
                {customFaviconUrl && (
                  <button
                    ...
                    disabled={isLoading}
                    aria-label={t('radio.deleteFavicon')}
                    title={t('radio.deleteFavicon')}
                  >
                    <Trash2 size={14} />
                  </button>
                )}
