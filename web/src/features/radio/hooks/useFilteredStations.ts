...
export function useFilteredStations({
  ...
  const {
    data: topVotedStations = [],
    isLoading: loadingTopVoted,
    error: errorTopVoted,
  } = useTopVotedStations(stationsPerView, isAllCountries && isTopFilter);

  const {
    data: countryTopStations = [],
    isLoading: loadingCountryTop,
    error: errorCountryTop,
  } = useStationsByCountry(!isAllCountries && isTopFilter ? country : '', stationsPerView);

  const {
    data: allCountryStations = [],
    isLoading: loadingAllCountry,
    error: errorAllCountry,
  } = useStationsByCountry(!isAllCountries && isAllFilter ? country : '', 10000);

  const {
    data: allWorldStations = [],
    isLoading: loadingAllWorld,
    error: errorAllWorld,
  } = useSearchStations(
    {
      limit: 10000,
  ...
  const {
    data: genreCountryStations = [],
    isLoading: loadingGenreCountry,
    error: errorGenreCountry,
  } = useSearchStations(
    {
  ...
  const {
    data: genreGlobalStations = [],
    isLoading: loadingGenreGlobal,
    error: errorGenreGlobal,
  } = useStationsByTag(isGenreFilter && isAllCountries ? filter : '', 10000);
