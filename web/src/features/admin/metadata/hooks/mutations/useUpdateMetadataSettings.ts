...
import { updateSetting } from '../../api/metadata.service';
  ...
  return useMutation({
    mutationFn: async (updates: UpdateSettingsPayload) => {
      ...
      if (updates.providers?.lastfm?.apiKey !== undefined) {
        promises.push(updateSetting('metadata.lastfm.api_key', updates.providers.lastfm.apiKey));
      }

      // Update Fanart.tv API key
      if (updates.providers?.fanart?.apiKey !== undefined) {
        promises.push(updateSetting('metadata.fanart.api_key', updates.providers.fanart.apiKey));
      }

      // Update storage mode
      if (updates.storage?.mode !== undefined) {
        promises.push(updateSetting('metadata.storage.location', updates.storage.mode));
      }

      // Update storage path
      if (updates.storage?.path !== undefined) {
        promises.push(updateSetting('metadata.storage.path', updates.storage.path));
      }
      ...
      if (updates.autoSearch?.confidenceThreshold !== undefined) {
        promises.push(
          updateSetting(
            'metadata.mbid_auto_search.confidence_threshold',
            updates.autoSearch.confidenceThreshold.toString()
          )
        );
      }
      ...
      if (updates.autoSearch?.autoApply !== undefined) {
        promises.push(
          updateSetting(
            'metadata.mbid_auto_search.auto_apply',
            updates.autoSearch.autoApply.toString()
          )
        );
      }
