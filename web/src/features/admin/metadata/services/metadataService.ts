...
import type { MetadataSettings, SettingDTO } from '../types/metadata.types';
...
export const metadataService = {
  ...
  validateApiKeyFormat(service: string, key: string): { valid: boolean; message: string } {
    if (!key.trim()) {
      return { valid: false, message: i18n.t('admin.metadata.apiKeyRequired') };
    }

    if (key.length < 10) {
      return { valid: false, message: i18n.t('admin.metadata.apiKeyTooShort') };
    }

    // Service-specific validations
    if (service === 'lastfm' && key.length !== 32) {
      return { valid: false, message: i18n.t('admin.metadata.lastfmKeyLength') };
    }
  ...
  validateStoragePathFormat(path: string): { valid: boolean; message: string } {
    if (!path.trim()) {
      return { valid: false, message: i18n.t('admin.metadata.pathRequired') };
    }

    if (path.includes('..')) {
      return { valid: false, message: i18n.t('admin.metadata.pathNoDotDot') };
    }

    if (!path.startsWith('/')) {
      return { valid: false, message: i18n.t('admin.metadata.pathMustBeAbsolute') };
    }
  ...
  getConfidenceDescription(threshold: number): string {
    if (threshold >= 0.95) return i18n.t('admin.metadata.confidenceVeryHigh');
    if (threshold >= 0.85) return i18n.t('admin.metadata.confidenceHigh');
    if (threshold >= 0.75) return i18n.t('admin.metadata.confidenceMedium');
    if (threshold >= 0.6) return i18n.t('admin.metadata.confidenceLow');
    return i18n.t('admin.metadata.confidenceVeryLow');
  },
