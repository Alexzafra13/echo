 ...
 */
export function buildImageUrl(entityId: string, imageType: ImageType, tag?: string): string {
...
export function validateImageFile(file: File): { valid: boolean; message: string } {
  ...
  if (!validTypes.includes(file.type)) {
    return {
      valid: false,
      message: i18n.t('admin.metadata.invalidFormat'),
    };
  }
  ...
  if (file.size > maxSize) {
    return {
      valid: false,
      message: i18n.t('admin.metadata.fileTooLarge'),
    };
  }
