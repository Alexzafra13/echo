...
import { radioFaviconsApi } from './radio-favicons.service';
...
describe('radioFaviconsApi', () => {
  ...
  describe('deleteFavicon', () => {
    it('debería enviar DELETE al endpoint correcto', async () => {
      ...
      expect(apiClient.delete).toHaveBeenCalledWith('/admin/radio/favicons/test-uuid');
    });
  });
  ...
  describe('autoFetch', () => {
    it('debería enviar POST con query params de nombre y homepage', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith(expect.stringContaining('name=Radio+Station'));
      expect(apiClient.post).toHaveBeenCalledWith(expect.stringContaining('homepage='));
      expect(result.success).toBe(true);
    });
