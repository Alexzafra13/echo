...
import { enrichmentApi, ListEnrichmentLogsFilters } from '../api/enrichment.service';
...
export function useEnrichmentStats(period?: 'today' | 'week' | 'month' | 'all') {
  return useQuery({
 ...
 */
export function useEnrichmentBackfill(
  stats: { byEntityType?: { artist: number; album: number } } | undefined
) {
  ...
  useEffect(() => {
    ...
    const hasArtistOrAlbumLogs =
      (stats.byEntityType?.artist ?? 0) > 0 || (stats.byEntityType?.album ?? 0) > 0;
