 ...
 */
function buildImageUrl(value: string | undefined, conflict: MetadataConflict): string | undefined {
...
export function ConflictCard({ conflict }: ConflictCardProps) {
  ...
  const FIELD_LABELS: Record<string, string> = {
    externalCover: t('admin.metadata.fieldExternalCover'),
    cover: t('admin.metadata.fieldCover'),
    biography: t('admin.metadata.fieldBiography'),
    images: t('admin.metadata.fieldImages'),
    year: t('admin.metadata.fieldYear'),
  };
  ...
  const handleAccept = () => {
    ...
    accept(conflict.id, {
      ...
      onError: (err) =>
        setError(t('admin.metadata.errorAccepting', { message: (err as Error).message })),
    });
  };
  ...
  const handleReject = () => {
    ...
    reject(conflict.id, {
      onSuccess: () => setIsRemoved(true),
      onError: (err) =>
        setError(t('admin.metadata.errorRejecting', { message: (err as Error).message })),
    });
  };
  ...
  const handleIgnore = () => {
    ...
    ignore(conflict.id, {
      onSuccess: () => setIsRemoved(true),
      onError: (err) =>
        setError(t('admin.metadata.errorIgnoring', { message: (err as Error).message })),
    });
  };
  ...
  const handleApplySuggestion = () => {
    ...
    applySuggestion(
      { conflictId: conflict.id, suggestionIndex: selectedSuggestionIndex },
      {
        ...
        onError: (err) =>
          setError(
            t('admin.metadata.errorApplyingSuggestion', { message: (err as Error).message })
          ),
      }
    );
  };
      ...
      <div className={styles.conflictCardHeader}>
        <div className={styles.conflictCardTitle}>
          <span className={styles.entityName}>
            {conflict.entity?.name || t('admin.metadata.unknown')}
          </span>
        ...
        <div className={styles.qualityNotices}>
          {conflict.metadata?.qualityImprovement && (
            <div className={styles.qualityBadge}>
              <Check size={14} />
              <span>{t('admin.metadata.qualityImprovement')}</span>
            </div>
          )}
          {conflict.metadata?.isLowQuality && (
            <div className={styles.lowQualityBadge}>
              <AlertCircle size={14} />
              <span>{t('admin.metadata.lowResolution')}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className={styles.conflictCardActions}>
        {hasMultipleSuggestions ? (
          <>
            ...
            >
              {t('admin.metadata.applySelection')}
            </Button>
            <Button
              variant="outline"
              size="sm"
