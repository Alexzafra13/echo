...
// Suppress unhandled rejections from React Query mutations in error tests
...
function renderWithClient(ui: React.ReactElement) {
  const queryClient = new QueryClient({
    defaultOptions: {
      ...
      mutations: {
        retry: false,
        // Prevent unhandled rejection errors in tests that mock rejected mutations
        onError: () => {},
      },
    },
  });
...
describe('MissingFilesPanel', () => {
  ...
  describe('loading state', () => {
    it('should show loading state initially', async () => {
      ...
      renderWithClient(<MissingFilesPanel />);
  ...
  describe('purge mode labels', () => {
    ...
    it('should show days for after_days mode', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Eliminar después de 30 días')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('empty state', () => {
    it('should show empty state when no missing files', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Todos los archivos de tu biblioteca están presentes en disco')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('delete track', () => {
    ...
    it('should delete track when clicking delete button', async () => {
      ...
      await waitFor(() => {
        expect(deleteMissingTrack).toHaveBeenCalledWith('track-1', expect.anything());
      });
    });
  ...
  describe('purge all', () => {
    ...
    it('should purge all tracks when confirming', async () => {
      ...
      const confirmButton = screen
        ...
        .find(
          (btn) =>
            btn.textContent === 'Purgar Todos' && btn.closest('[data-testid="confirm-dialog"]')
        );
      if (confirmButton) {
  ...
  describe('settings modal', () => {
    ...
    it('should open settings modal when clicking settings button', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTitle('Configuración de purga')).toBeInTheDocument();
      });

      fireEvent.click(screen.getByTitle('Configuración de purga'));

      await waitFor(() => {
        expect(screen.getByText('Configuración de Purga')).toBeInTheDocument();
      });
    });
    ...
    it('should show purge mode options', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Mantener tracks desaparecidos indefinidamente')
        ).toBeInTheDocument();
        expect(
    ...
    it('should close modal when clicking cancel', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('Configuración de Purga')).not.toBeInTheDocument();
      });
    });
