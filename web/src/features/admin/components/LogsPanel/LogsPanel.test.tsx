...
describe('LogsPanel', () => {
  ...
  describe('Pagination', () => {
    it('should render pagination when total > limit', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Mostrando 1-3 de 25')).toBeInTheDocument();
      });
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should dismiss error notification', async () => {
      ...
      // After changing a filter (which resets the query key), the error clears.
      let logsCallCount = 0;
      mockGet.mockImplementation((url: string) => {
        if (url === '/logs/retention') {
          return Promise.resolve({ data: { retentionDays: 30 } });
      ...
      // Change filter to trigger a new query (new query key clears old error)
      const levelSelect = screen.getAllByRole('combobox')[1];
      fireEvent.change(levelSelect, { target: { value: 'error' } });
