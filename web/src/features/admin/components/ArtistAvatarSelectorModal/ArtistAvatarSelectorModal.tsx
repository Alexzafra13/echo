...
import { AvatarOption, AvatarImageType } from '../../api/artist-avatars.service';
...
export function ArtistAvatarSelectorModal({
  ...
  const types = allowedTypes
    ? allTypes.filter(
        (type): type is AvatarImageType => type !== undefined && allowedTypes.includes(type)
      )
    : allTypes;
    ...
    applyAvatar(
      ...
      },
      {
        ...
        onError: (err) => {
          ...
          setApplyError(getApiErrorMessage(err, t('artists.errorApplyingImage')));
        },
      }
    );
  };
  ...
  const getTypeLabel = (type: string) => {
    const labels: Record<string, string> = {
      profile: t('artists.typeProfile'),
      background: t('artists.typeBackground'),
      banner: t('artists.typeBanner'),
      logo: t('artists.typeLogo'),
    };
    ...
    <div className={styles.modal} onClick={onClose}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <div>
            <h2 className={styles.title}>{t('artists.selectArtistImage')}</h2>
          ...
          {activeTab === 'providers' ? (
            isLoading ? (
              <div className={styles.loading}>
                ...
                <p>{t('artists.searchingProviders')}</p>
              </div>
            ) : error ? (
              <div className={styles.error}>
                <AlertCircle size={48} />
                <p>{t('artists.errorSearchingImages')}</p>
                <span>{(error as Error).message}</span>
              </div>
            ) : avatars.length === 0 ? (
              <div className={styles.empty}>
                <AlertCircle size={48} />
                <p>{t('artists.noImagesFound')}</p>
                <span>{t('artists.noImagesAvailable')}</span>
              </div>
            ) : (
              <>
                <div className={styles.filterSection}>
                  {types.length > 1 && (
                    <div className={styles.filterGroup}>
                      <label className={styles.filterLabel}>{t('artists.typeFilterLabel')}</label>
                      <select
                      ...
                      >
                        <option value="">
                          {t('artists.allWithCount', { count: avatars.length })}
                        </option>
                        {types.map((type) => (
                  ...
                  {providers.length > 1 && (
                    <div className={styles.filterGroup}>
                      ...
                      >
                        <option value="">{t('artists.allFilter')}</option>
                ...
                <div className={styles.gallery}>
                  {filteredAvatars.map((avatar, index) => (
                    ...
                    >
                      ...
                      <div
                        className={`${styles.imageWrapper} ${brokenUrls.has(avatar.url) ? styles.imageUnavailable : ''}`}
                      >
                      ...
                      <div className={styles.avatarInfo}>
                        {avatar.type && (
                          <span className={`${styles.typeBadge} ${getTypeBadgeColor(avatar.type)}`}>
