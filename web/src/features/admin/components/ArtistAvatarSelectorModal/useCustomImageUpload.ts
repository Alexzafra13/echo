...
import { CustomImage, AvatarImageType } from '../../api/artist-avatars.service';
...
export function useCustomImageUpload({
  artistId,
  imageType,
  onSuccess,
}: UseCustomImageUploadOptions) {
  ...
  const { data: customImagesData, isLoading: isLoadingCustomImages } =
    useListCustomImages(artistId);
