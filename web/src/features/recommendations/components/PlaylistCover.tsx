...
function EchoBadge({ className }: { className?: string }) {
  return (
    <img src="/images/logos/echo_dark.svg" alt="Echo" className={className} draggable={false} />
  );
}
    ...
    <div className={`${styles.cover} ${styles[size]} ${className}`}>
      {showImage ? (
        ...
        <div
          ...
          style={genreGradient ? { background: genreGradient } : { backgroundColor }}
        >
          {isGenre ? (
            <>
              {genreOverlayUrl && (
              ...
              <div className={styles.genreName}>{genreDisplayName}</div>
            </>
          ) : type === 'artist' ? (
            <>
              <EchoBadge className={styles.logoBadge} />
              {artistName && <div className={styles.genreName}>{artistName}</div>}
            </>
          ) : (
            <>
              ...
              {type === 'wave-mix' && (
                <div className={styles.coverText}>{t('recommendations.dailyRecommendations')}</div>
              )}
            </>
          )}
        </div>
