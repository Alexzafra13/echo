...
export function useWaveMixPlaylists() {
  ...
  const loadPlaylists = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, i18next.t('recommendations.errorLoadingPlaylists')));
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, i18next.t('recommendations.errorUpdatingPlaylists')));
    } finally {
      setIsLoading(false);
    }
  }, []);
