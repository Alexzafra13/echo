...
export function WaveMixPage() {
  ...
  // Fallback: álbumes recientes para usuarios nuevos sin historial
  const [fallbackCovers, setFallbackCovers] = useState<string[]>([]);

  // Recopilar URLs de portadas de álbum para el mosaico animado
  const heroCoverRows = useMemo(() => {
    const MAX_HERO_COVERS = 40;
    ...
    // Si hay suficientes del historial, usarlas
    if (urls.length >= 8) {
      const mid = Math.ceil(urls.length / 2);
      return { row1: urls.slice(0, mid), row2: urls.slice(mid) };
    }

    // Fallback: usar álbumes recientes de la biblioteca
    if (fallbackCovers.length >= 8) {
      const mid = Math.ceil(fallbackCovers.length / 2);
      return { row1: fallbackCovers.slice(0, mid), row2: fallbackCovers.slice(mid) };
    }

    return { row1: [], row2: [] };
  }, [dailyPlaylists, artistPlaylists, genrePlaylists, fallbackCovers]);

  // Cargar álbumes recientes si no hay portadas del historial
  useEffect(() => {
    if (heroCoverRows.row1.length > 0 || isLoading) return;
    const loadFallback = async () => {
      try {
        const { apiClient } = await import('@shared/services/api');
        const { data } = await apiClient.get('/albums', {
          params: { limit: 40, sort: 'recently_added' },
        });
        const albums = (data as { data?: { id: string }[] }).data ?? [];
        const urls = albums.map(
          (a: { id: string }) => `/api/images/albums/${a.id}/cover?size=thumb`
        );
        if (urls.length >= 8) setFallbackCovers(urls);
      } catch {
        /* sin portadas de fallback */
      }
    };
    loadFallback();
  }, [heroCoverRows.row1.length, isLoading]);
    ...
    <div className={styles.waveMixPage}>
      ...
      <main className={styles.waveMixPage__main}>
        ...
        <div className={styles.waveMixPage__content}>
          {/* Hero Section */}
          <div
            className={`${styles.hero} ${heroCoverRows.row1.length === 0 ? styles['hero--fallback'] : ''}`}
          >
            ...
            {(heroCoverRows.row1.length > 0 || heroCoverRows.row2.length > 0) && (
              <div className={styles.hero__mosaic}>
                {[heroCoverRows.row1, heroCoverRows.row2].map((row, rowIdx) => {
                  // Repetir suficientes veces para cubrir el ancho (mínimo 30 imágenes por track)
                  const repeats = row.length > 0 ? Math.max(2, Math.ceil(30 / row.length)) : 0;
                  const expandedRow = Array.from({ length: repeats }, () => row).flat();
                  return (
                    <div key={rowIdx} className={styles.hero__row}>
                      <div
                        className={styles.hero__track}
                        data-direction={rowIdx === 0 ? 'left' : 'right'}
                      >
                        {expandedRow.map((url, i) => (
                          <img
                            key={i}
                            src={url}
                            alt=""
                            className={styles.hero__coverImg}
                            loading="eager"
                            decoding="async"
                            fetchPriority="low"
                          />
            ...
            <div className={styles.hero__content}>
              <div className={styles.hero__info}>
                <h1 className={styles.hero__title}>{t('recommendations.waveMixTitle')}</h1>
                <p className={styles.hero__subtitle}>
                  {t('recommendations.waveMixSubtitle', {
              ...
              <div className={styles.hero__right}>
                  ...
                  <div className={styles.hero__stats}>
                    <div className={styles.hero__stat}>
                      ...
                      <span className={styles.hero__statValue}>
                        {playlists.reduce((acc, p) => acc + (p.metadata.totalTracks || 0), 0)}
                      </span>
                      <span className={styles.hero__statLabel}>
                        {t('recommendations.songsLabel')}
                      </span>
                    </div>
                    ...
                    <div className={styles.hero__stat}>
                      ...
                      <span className={styles.hero__statLabel}>
                        {t('recommendations.artistsLabel')}
                      </span>
                    </div>
                    <div className={styles.hero__statDivider} />
                    <div className={styles.hero__stat}>
                ...
                >
                  ...
                  <span className={styles.hero__refreshBtnText}>
                    {isLoading ? t('recommendations.updating') : t('recommendations.update')}
                  </span>
                </Button>
              </div>
            </div>
          ...
          {isLoading && (
            <div className={styles.waveMixPage__loading}>
              ...
              <p>{t('recommendations.generatingPlaylists')}</p>
            </div>
          )}
          ...
          {!isLoading && !error && playlists.length === 0 && (
            <div className={styles.waveMixPage__emptyState}>
              <Waves size={64} />
              <h2>{t('recommendations.noPlaylists')}</h2>
              <p>{t('recommendations.noPlaylistsHint')}</p>
            </div>
          )}
            ...
            <>
              ...
              {dailyPlaylists.length > 0 && (
                <div className={styles.waveMixPage__section}>
                  <h2 className={styles.waveMixPage__sectionTitle}>
                    {t('recommendations.dailyRecommendations')}
                  </h2>
              ...
              {artistPlaylists.length > 0 && (
                <div className={styles.waveMixPage__section}>
                  <h2 className={styles.waveMixPage__sectionTitle}>
                    {t('recommendations.artistRecommendations')}
                  </h2>
                    ...
                    {artistPlaylists.slice(0, neededItems).map((playlist) => (
                      ...
                      >
                        ...
                        <div className={styles.playlistCard__info}>
                          <h3 className={styles.playlistCard__name}>
              ...
              {genrePlaylists.length > 0 && (
                <div className={styles.waveMixPage__section}>
                  <h2 className={styles.waveMixPage__sectionTitle}>
                    {t('recommendations.genreRecommendations')}
                  </h2>
                  <div className={styles.waveMixPage__viewAllButtonWrapper}>
                    <button
