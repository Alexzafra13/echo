...
vi.mock('@shared/components/TrackList', async () => {
  const playerCtx = await import('@features/player');
  return {
    TrackList: ({
      tracks,
      onTrackPlay,
      ...
      const { isPlaying, pause, play } = playerCtx.usePlayback();
      return (
...
// playerContext removed — now using mockPlaybackState/mockQueueState directly
...
describe('SharedAlbumPage', () => {
  ...
  describe('track list', () => {
    ...
    it('should toggle play/pause when clicking currently playing track', () => {
      mockPlaybackState.currentTrack = { id: 'server-1-track-1' };
