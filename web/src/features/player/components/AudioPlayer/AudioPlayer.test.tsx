...
vi.mock('../../context/PlaybackContext', () => ({
  usePlayback: () => mockPlayerContext,
}));

vi.mock('../../context/QueueContext', () => ({
  useQueue: () => mockPlayerContext,
}));

vi.mock('../../context/RadioContext', () => ({
  useRadio: () => mockPlayerContext,
}));

// Mock timeStore para inyectar currentTime/duration en tests
vi.mock('../../store/timeStore', () => ({
  useCurrentTime: () => ({
    currentTime: mockPlayerContext.currentTime,
    duration: mockPlayerContext.duration,
  }),
}));
...
vi.mock('@features/music-videos/components/VideoPlayer/VideoPlayer', () => ({
  VideoPlayer: () => null,
}));

vi.mock('@features/music-videos/services/music-videos.service', () => ({
  musicVideosService: {
    getStreamUrl: vi.fn().mockReturnValue('/api/music-videos/mock/stream?token=mock'),
  },
}));
...
vi.mock('../NowPlayingView', () => ({
  NowPlayingView: ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) =>
    isOpen ? (
  ...
  describe('album navigation', () => {
    beforeEach(() => {
      mockPlayerContext.currentTrack = {
        ...
        artist: 'Test Artist',
        albumId: 'album-1',
        album: { cover: '/covers/test.jpg', name: 'Test Album' } as {
          cover: string;
          name?: string;
        },
        duration: 180,
