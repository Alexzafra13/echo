...
describe('useRadioPlayback', () => {
  ...
  describe('initial state', () => {
    it('should start with no radio mode', () => {
      const { result } = renderHook(() => useRadioPlayback({ audioElements }));
  ...
  describe('stopRadio', () => {
    ...
    it('should call stopBoth and resetToAudioA when in radio mode', async () => {
      const { result } = renderHook(() => useRadioPlayback({ audioElements }));

      // Entrar en radio mode primero
      await act(async () => {
        await result.current.playRadio(createRadioStation());
      });

      (audioElements.stopBoth as ReturnType<typeof vi.fn>).mockClear();
      (audioElements.resetToAudioA as ReturnType<typeof vi.fn>).mockClear();
    ...
    it('should NOT call stopBoth when not in radio mode', async () => {
      const { result } = renderHook(() => useRadioPlayback({ audioElements }));
      ...
      // No estaba en radio mode → no debe matar el audio activo
      expect(audioElements.stopBoth).not.toHaveBeenCalled();
    });
  });
