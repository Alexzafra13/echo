...
export function useTrackPlayback({
  ...
  /**
   * Play a track con decisión de crossfade vs normal.
   *
   * IMPORTANTE sobre radio: NO llamamos radio.stopRadio() aquí. La radio
   * se para en playQueue()/playRadio() ANTES de llamar a playTrack().
   * stopRadio() hace stopBoth() que vacía ambos audio elements; si se
   * ejecuta aquí (después de getStreamUrl async), mata el audio que
   * playNormalTrack acaba de cargar — causando silencio.
   *
   * El nuevo src asignado por loadOnActive() implícitamente detiene
   * cualquier stream previo (radio o música) en ese audio element.
   */
  const playTrack = useCallback(
    async (track: Track, withCrossfade: boolean = false) => {
      ...
      // Decidir crossfade: solo si fue pedido explícitamente, está habilitado,
      // y hay audio activo reproduciéndose (NO si venimos de parar la radio).
      // Usar isCrossfadingRef como guard síncrono: si onCrossfadeTrigger ya
      // lo setó, DEBEMOS tomar el path de crossfade aunque isPlaying sea stale.
      const shouldCrossfade =
        withCrossfade &&
        crossfadeSettings.enabled &&
        !radio.isRadioMode && // Nunca crossfade desde/hacia radio
