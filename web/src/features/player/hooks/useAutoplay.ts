...
interface AutoplayState {
  ...
  // Incrementa cuando load empieza, para invalidar prefetch en vuelo
...
export function useAutoplay() {
   ...
   */
  const convertToPlayerTrack = useCallback(
    (track: {
      id: string;
      title: string;
      artistName?: string;
      ...
      return {
        ...
        coverImage: track.albumId
          ? `${API_BASE_URL}/images/albums/${track.albumId}/cover`
          : undefined,
      };
    },
    []
   ...
   */
  const fetchSimilarArtistTracks = useCallback(
    async (
      ...
      try {
        ...
        logger.debug(`${logPrefix} Searching for related artists for:`, currentArtistId);
        const relatedResponse = await artistsService.getRelatedArtists(
          currentArtistId,
          MAX_ARTISTS_TO_TRY
        );

        logger.debug(`${logPrefix} Related artists response:`, {
        ...
        if (!relatedResponse.data || relatedResponse.data.length === 0) {
          logger.debug(`${logPrefix} No related artists found`);
          ...
          try {
            ...
            if (playlist.tracks && playlist.tracks.length > 0) {
              ...
              if (freshTracks.length > 0) {
                ...
                logger.debug(
                  `${logPrefix} Added ${freshTracks.length} tracks from ${relatedArtist.name}`
                );
              }
            }
          } catch (err) {
            logger.debug(`${logPrefix} Failed to get tracks for ${relatedArtist.name}:`, err);
          }
        }
   ...
   */
  const prefetchSimilarArtistTracks = useCallback(
    async (currentArtistId: string, excludeTrackIds: Set<string> = new Set()): Promise<void> => {
      const state = stateRef.current;
      ...
      if (
        state.prefetchedResult?.artistId === currentArtistId &&
        state.prefetchedResult.tracks.length > 0
      ) {
      ...
      // Capturar generation para detectar si load se disparo durante el prefetch
      const generation = state.prefetchGeneration;
      logger.debug('[Autoplay Prefetch] Starting prefetch for artist:', currentArtistId);

      try {
        const result = await fetchSimilarArtistTracks(currentArtistId, excludeTrackIds, true);

        // Si load se ejecuto mientras prefetch estaba en vuelo, descartar resultado
        if (state.prefetchGeneration !== generation) {
          logger.debug('[Autoplay Prefetch] Discarded — load started during prefetch');
          return;
        }
        ...
        if (result.tracks.length > 0) {
          ...
          logger.debug(
            `[Autoplay Prefetch] Cached ${result.tracks.length} tracks for instant playback`
          );
        }
      } finally {
   ...
   */
  const loadSimilarArtistTracks = useCallback(
    async (
      currentArtistId: string,
      excludeTrackIds: Set<string> = new Set()
    ): Promise<{ tracks: Track[]; sourceArtistName: string | null }> => {
      ...
      if (
        ...
        logger.debug('[Autoplay] Using prefetched tracks - instant playback!');
      ...
      // No prefetched data, fetch now. Invalidar prefetch en vuelo.
      state.loading = true;
      state.prefetchGeneration++;
      logger.debug('[Autoplay] No prefetched data, fetching now...');
