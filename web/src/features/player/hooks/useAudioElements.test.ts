...
describe('useAudioElements', () => {
  ...
  function audioA() {
    return mockAudioInstances[mockAudioInstances.length - 2];
  }
  function audioB() {
    return mockAudioInstances[mockAudioInstances.length - 1];
  }
  ...
  describe('switchActiveAudio', () => {
    ...
    it('devuelve el nuevo id activo', () => {
      ...
      let newId: 'A' | 'B';
      act(() => {
        newId = result.current.switchActiveAudio();
      });
      expect(newId!).toBe('B');
    });
  });
  ...
  describe('loadOnActive / loadOnInactive', () => {
    ...
    it('carga fuente en el audio inactivo con volumen 0', () => {
      const { result } = renderHook(() => useAudioElements());
      act(() => {
        result.current.loadOnInactive('http://example.com/next.mp3');
      });
  ...
  describe('stopBoth', () => {
    it('para ambos elementos sin limpiar src (preserva autoplay permission)', async () => {
      const { result } = renderHook(() => useAudioElements());
      ...
      expect(audioA().pause).toHaveBeenCalled();
      expect(audioB().pause).toHaveBeenCalled();
      // src NO se limpia para preservar autoplay permission en mobile
  ...
  describe('stopActive / stopInactive', () => {
    it('para el audio activo sin limpiar src', async () => {
      const { result } = renderHook(() => useAudioElements());
