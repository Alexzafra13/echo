...
test.describe('Panel de Administración', () => {
  test.describe('Acceso y navegación', () => {
    ...
    test('muestra las tabs de navegación', async ({ page }) => {
      ...
      await expect(sidebar.getByText(/Librería|Library/i).first()).toBeVisible({ timeout: 5000 });
      await expect(sidebar.getByText('Metadata')).toBeVisible();
      await expect(sidebar.getByText(/Mantenimiento|Maintenance/i)).toBeVisible();
      await expect(sidebar.getByText(/Usuarios|Users/i).first()).toBeVisible();
    ...
    test('navega entre tabs por URL', async ({ page }) => {
      ...
      await expect(page.getByText(/Logs|Sistema|System/i).first()).toBeVisible({ timeout: 15000 });
    });
  ...
  test.describe('Panel de Usuarios', () => {
    ...
    test('abre el modal de crear usuario', async ({ page }) => {
      const createButton = page.getByRole('button', { name: /Crear Usuario|Create User/i });
  ...
  test.describe('Panel de Mantenimiento', () => {
    test('carga la pestaña de mantenimiento', async ({ page }) => {
      ...
      await expect(
        page.getByText(/Almacenamiento|Storage|Limpieza|Cleanup|Cargando|Loading/i).first()
    ...
    test.skip('la notificación se cierra automáticamente', async ({ page }) => {
      ...
      if (await saveButton.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const notification = page.locator('[class*="notification"]').filter({
          hasText: /guardad|correctamente|saved|success/i,
        });
