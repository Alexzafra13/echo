...
# Uso: ./scripts/setup.sh [--skip-docker] [--skip-frontend]
...
# Start
print_header "ECHO - Instalación Inicial"
...
if check_command node; then
  ...
  HAS_ERROR=true
  print_error "Instala Node.js >= 18 desde: https://nodejs.org"
fi
...
if [ "$SKIP_BACKEND" = false ]; then
  ...
  if [ -f "package.json" ]; then
    ...
    print_error "No se encuentra package.json en la raíz"
    exit 1
  fi
else
...
if [ ! -f ".env" ]; then
  if [ -f ".env.development.example" ]; then
    ...
    print_warning "IMPORTANTE: Revisa el archivo .env y ajusta si es necesario"
    print_info "Debe tener: DATABASE_URL con 'localhost' (no 'postgres')"
  else
...
if [ "$SKIP_FRONTEND" = false ]; then
  ...
  if [ -d "frontend" ] && [ -f "frontend/package.json" ]; then
    print_info "Ejecutando: cd frontend && pnpm install"
    cd frontend
    pnpm install
    cd ..
...
echo ""
echo "  Backend:"
echo "    pnpm start:dev        # Iniciar servidor de desarrollo"
...
if [ "$SKIP_FRONTEND" = false ]; then
  echo "  Frontend:"
  echo "    cd frontend"
  echo "    pnpm dev             # Iniciar frontend en http://localhost:5173"
  echo ""
fi
...
echo "    DOCKER_COMPOSE_INFO.md  # Guía de Docker"
echo -e "${NC}"
